...
1. **SOC Ef 決定** — `lmf --quit=band -vnspin=2 --phispinsym -vso=1`
   - フル LS のスピノルハミルトニアンを全 BZ メッシュで解き `efermi.lmf` を生成
   - これを `efermi_soc` にコピー（以後の lmf 実行でも上書きされない）
2. **H + V_SO 書き出し** — `lmf --writeham --mkprocar --noinv --mlo -vnspin=2 --phispinsym -vso=0 --socmatrix`
...
- `bnd00N.spin1`: SOC spinor DFT bands (`job_band -vnspin=2 -vso=1 --phispinsym`)
...
- **黒線**: DFT-SOC bands (`job_band -vnspin=2 -vso=1` 出力)
