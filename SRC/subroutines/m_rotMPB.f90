...
contains
  subroutine RotMPB2(nbloch,ngbb,qin,igx,igxt,ginv, zrotm) !zcousqr=Rotate_igx(zcousq) igxt=-1 means timereversal case.
    ...
    ! (2022jan): for igxt=-1, t.kotani think we need to reexamine the code.
    ...
    call rotmto2(qin,nbloch,ngbb, &
         ...
         symops(:,:,igx),shtvg(:,igx),dlmm(:,:,:,igx),lxxa,miat(:,igx),tiat(:,:,igx),igxt,nbas, &
         zrotm(1:nbloch,1:nbloch))
    ...
    call rotipw2(qin,qout,ngcx,ngbb, &
         ...
         zrotm(nbloch+1:nbloch+ngcx,nbloch+1:nbloch+ngcx) )
