     ...
     .ndimh,iprmb,isp,evl,evec,ewgt,cPkL,db,!da,wk,
     .sigpp,sighp,ppippz,ppihpz,f) !lcplxp,
...
Ci   lcplxp=1 only now: ppi is complex
Co Outputs
...
c     .ppihp(nkaph,0:kmax,nlmha,nlma,isp)
c     .ppipp(0:kmax,0:kmax,nlma,nlma,isp)
...
c     .ppipp,ppippz,sigpp,ppihp,ppihpz,sighp,cPkL,wk)!lcplxp,
...
!   ... ppi contribution: loop over largest blocks possible
        if (blks(io) .ne. 0) then ! .and. lcplxp .eq. 1) then
          nlm2 = nlm1 + blks(io)-1
          i  = offl(io)
          do  ilmb = nlm1, nlm2
            i = i+1
