subroutine qmpkl(rmax,rsm,k0,kmax,lmax,pmax,aklm,qmp,qval)
  !- Integrals of pkl r^(2+2*l+p) from 0 to rmax, and value of pkl r^(l+p)
  !  at rmax, p=0..pmax.
  !     implicit none
  ...
  ! --- Decomposition of pkl into simple polynomials ---
  call pkl2r(rsm,k0,kmax,lmax,aklm)

  ! --- Integrate the pkl term by term ---
  do  101  p = 0, pmax
     do  10  l = 0, lmax
        do  20  k = 0, kmax
           sumq = 0
...
10   enddo
101 enddo
end subroutine qmpkl

subroutine spkli(rmax,rsm,k0,kmax,lmax,pmax,aklm,s)
  !- Integrals of pkl pk'l r^(2+2l+p) from 0 to rmax, p=0..pmax
  !     implicit none
  integer :: k0,kmax,lmax,pmax,l,p
  double precision :: rmax,rsm,aklm(0:k0,0:k0,0:lmax), &
  ...
  ! --- Integrate the pkl pk'l product term by term ---
  do  101  p = 0, pmax
     do  10  l = 0, lmax
        do  201  k1 = 0, kmax
           do  20  k2 = 0, kmax
...
10   enddo
101 enddo
end subroutine spkli
subroutine tpkli(rmax,rsm,k0,kmax,lmax,aklm,s)
  !- Integrals of pkl (-nabla^2 pk'l) r^(2+2l) from 0 to rmax
  !  Integrates (pkl 1/r d^2(r*pk'l) - l(l+1)/r^2*pkl*pk'l) r^(2+2l)
  !  from r=0 to rmax
  !     implicit none
  ...
  ! --- Integrate -(-l(l+1)r**-2 pkl pk'l) r^2 ---
  do  10  l = 0, lmax
     do  121  k1 = 0, kmax
        do  12  k2 = 0, kmax
           sum = 0d0
...
10 enddo
  ...
  ! --- Integrate pkl -1/r d^2(r*pk'l)/dr^2 r^2 ---
  do  20  l = 0, lmax
     do  221  k1 = 0, kmax
        do  22  k2 = 0, kmax
           sum = 0d0
...
end subroutine tpkli
subroutine s3pkl(rmax,rsma,k0,l0,km1,km2,km3,lm1,lm2,lm3,aklm,s)
  !- Integrals to rmax of products of three radial p_kl's
  !  Returns s(k1l1,k2l2,k3l3) where 0<k_j<km_j and 0<l_j<lm_j
  !     implicit none
  ...
  ! --- Integrate the pkl products term by term ---
  tar2 = 2d0/rsma**2
  do  12  l1 = 0, lm1
     do  11  l2 = 0, lm2
        do  10  l3 = 0, lm3
...
11   enddo
12 enddo
end subroutine s3pkl
subroutine s3pkl0(rmax,rsm,k0,kmax,kmax3,lmax,aklm,s)
  !- Integrals from r=0 to rmax of p_k1,l p_k2,l p_k3,0
  !  Integrals are: pkl pk'l p_k3,0 r^(2+2l), 0<k3<kmax3
  !     implicit none
...
22   enddo
10 enddo
end subroutine s3pkl0
subroutine s3pklx(rmax,rsm,rsm3,k0,kmax,kmax3,lmax,aklm,aklm3,s)
  !- Integrals from r=0 to rmax of p_k1,l p_k2,l p_k3,0
  !  Integrals are: pkl pk'l p_k3,0 r^(2+2l), 0<k3<kmax3
  !  Pkl for k=1,2 defined by rsm, for k=3 defined by rsm3
  !     implicit none
  ...
  !|    double precision p1(0:20,0:10),p2(0:20,0:10)
...
10 enddo
  ...
  !|              do i=2,n
  !|                r=(i-1)*dr
  !|                wgt=2*(mod(i+1,2)+1)/3d0
  !|                if (i.eq.n) wgt=1d0/3d0
  !|                call radpkl(r,rsm,kmax,lmax,20,p1)
  !|                call radpkl(r,rsm3,kmax3,0,20,p2)
  !|                sum=sum+dr*wgt*p1(k1,l)*p1(k2,l)*p2(k3,0)*r**(2*l+2)
  !|              enddo
  !|              write (6,890) l,k1,k2,k3,s(k1,k2,k3,l),sum
  !|  890         format(4i4,2f14.8)
  !|              s(k1,k2,k3,l)=sum
  !|            enddo
  !|          enddo
  !|        enddo
...
end subroutine s3pklx

subroutine pkl2r(rsm,k0,kmax,lmax,aklm)
  !- Coefficients that decompose pkl in simple polynomials
  !  pkl = sum_m=0..k akl(m,k,l) x^m, where x=2*(r/rsm)^2
  !     implicit none
...
21 enddo
  ...
  !      do  30  l = 0, lmax
  !      do  30  k = 0, kmax
  !   30 print 333, l,k,(aklm(m,k,l), m=0,kmax)
  !  333 format(2i4,10f12.6)

end subroutine pkl2r

subroutine phkl2r(rsm,k0,kmax,lmax,aklm)
  !- Coefficients that decompose phi_kl in simple polynomials
  !  phi_kl = sum_m=0..k akl(m,k,l) x^m, where x=2*(r/rsm)^2
...
end subroutine phkl2r

subroutine p2pkl(rsm,k0,mmax,lmax,aklm,cmkl)
  !- Decomposition of simple polynomials into P_kl
  !  Let F_ml = r**(2m+l)Y_L.  Then F_mL = sum_k=0..m c_pkl p_kl r^l Y_L
  !     implicit none
  ...
  ! --- Decomposition of phi_kl into simple polynomials ---
...
5 enddo
  ...
  ! --- Integrate r^(2m+l) Y_L phi_kl r^l Y_L g0 term by term ---
  !     and fold in scaling to make pkl expansion
  do  10  m = 0, mmax
     dfac = 1d0
     do  12  l = 0, lmax
        fack = 1d0
...
10 enddo
  ...
  !      do  30  l = 0, lmax
  !      do  30  m = 0, mmax
  !   30 print 333, l,m,(cmkl(m,k,l), k=0,mmax)
  !  333 format(2i4,10f12.6)

end subroutine p2pkl
