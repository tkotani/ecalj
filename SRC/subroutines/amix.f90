...
!!!!!!!!!!!!!!!! trying to replace to lapack. need fixing.
!     lwork=64*nmix
!     block
!       integer:: info,ldx,iwork(nmix)
!       real(8):: work(lwork),af(nmix,nmix),x(nmix,1),rcond,ferr(1),berr(1)
!       Call dsysvx('Not factored', 'Upper', nmix, 1, norm, mmix, af, nmix, &
!            nmix, x, nmix, rcond, ferr, berr, work, lwork, iwork, info)
!       if (abs(ferr(1))+abs(berr(1)) >1d-8) then !singular
!          t(nmix) = 0
!          nmix = nmix-1
...
!     endblock
    ...
    call dsifa(norm,mmix,nmix,kpvt,i) !i/=0 if singular norm matrix.
    if (i /= 0) then
       sing = sinmax + 1
    else
   ...
   ! --  Handle singular normal matrix --
    if (sing > sinmax) then
       ...
       nmix = nmix-1
       if (ipr >= 20) print 337, sinmax, nmix
337    format(' AMIX: condition of normal eqns >',f7.0,' Reducing nmix to',2i2)
       goto 10
    endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
