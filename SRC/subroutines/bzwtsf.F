...
Cioxxx  lwtkb :Used in connection w/ fixed spin-moments method.  On input:
...
c      if (fmom==NULLR .or. lwtkb .eq. 0) call rx('bzwtsf2: check logic of lwtkb')
...
c      if (fmom .eq. 0 .or. lwtkb .eq. 0) return
      if (fmom==NULLR .or. lwtkb .eq. 0) return
c      if (fmom==NULLR .or. lwtkb .eq. 0) call rx('bzwtsf:check logic of lwtkb')
      call tcn('bzwtsf')
...
C       A root was found
          if (vhold(12) .eq. -2 .or. vhold(12) .eq. -3 .or.
     .          vhold(12) .eq.  0 .or. vhold(12) .eq.  1) then
          if (ipr .ge. 10) write(stdo,ftox)' BZWTSF: potential shift bracketed.',
     .    'Unconstrained efermi=',ftof(ef0),
