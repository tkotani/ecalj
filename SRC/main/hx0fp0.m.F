...
!! (only epsPP_lmf_chipm mode works).
     ...
     &     alat, esmr,clabl,iclass, !diw,dw,delta,deltaw,
     &     il,in,im,nlnm, 
...
!! q0p
c      use m_readq0p,only: Readq0p,
c     &     wqt,q0i,nq0i ,nq0iadd,ixyz,nq0ix,neps
...
! svd. not used now
     ...
     &  ,epsmode=.false.,normalm=.false., crpa=.false. ,epsPPmode=.false.
...
c     &  w_k(:,:,:),w_ks(:,:,:),w_kI(:,:,:),w_ksI(:,:,:), llw(:,:), llwI(:,:),
     ...
     &  w_k(:),w_ks(:),w_kI(:), w_ksI(:), s_vc(:),vw_k(:),vw_ks(:)
      ...
      complex(8):: vc1vc2
c      integer,allocatable:: neibz(:),nwgt(:,:),ngrpt(:),igx(:,:,:),igxt(:,:,:),eibzsym(:,:,:)
...
c     nblochpmx = nbloch + ngcmx !rdpp \in mptauof_zmel \in m_zmel  
...
c      do 1000 iqxx=1,2          !for iq=1 at first for zme0mode
c         if(iqxx==2.and.(.not.zmel0mode)) exit
c         if(zmel0mode) call MPI__barrier()
      do 1001 iq = iqxini,iqxend  ! NOTE: q=(0,0,0) is omitted when iqxini=2
         if(zmel0mode) then
            if(iq==iqxini) cycle
c            if(iqxx==1.and.iq/=iqxini) cycle
c            if(iqxx==2.and.iq==iqxini) cycle
         endif   
...
c$$$          do icount = 1,ncount
c$$$            k = kc(icount)
c$$$            if(k/=kold.or.is/=isold) then
c$$$               call x0kf_zmel(q,      iq,k, is,isf)
c$$$               if(zmel0mode) then
c$$$                 call setzmel0()
c$$$                 call x0kf_zmel(q00,iqxini,k, is,isf)
c$$$                 call unsetzmel0()
c$$$                 q1a=sum(q00**2)**.5
c$$$                 q2a=sum(q**2)**.5
c$$$                 rfac00=q2a/(q2a-q1a)
c$$$               endif  
c$$$               kold=k
c$$$               isold=is
c$$$            endif   
...
c$$$              nini=nw_i
c$$$              nend=nw
c$$$              open(newunit=ifrcwx, file='WVR.'//i2char(iq), form='unformatted', access='direct',recl=mrecl)
c$$$           elseif(ircw==2) then
c$$$              nini=1
c$$$              nend=niw
c$$$              open(newunit=ifrcwx, file='WVI.'//i2char(iq), form='unformatted',access='direct',recl=mrecl)
c$$$            endif
c$$$            do iw=nini,nend
c$$$              read(ifrcwx, rec= iw-nini+1) zw 
c$$$              if(ircw==1) zw(1,1) = w0(iw)
c$$$              if(ircw==2) zw(1,1) = w0i(iw)
c$$$              write(ifrcwx,rec= iw-nini+1 ) zw 
c$$$            enddo
c$$$            close(ifrcwx)
c$$$          enddo
c$$$        enddo
