...
\abstract{
\noindent An all-electron full-potential electronic structure 
calculation package freely availalbe.\\
...
(Wannier functions also) based on the PMT=LA\underline{P}W+L\underline{MT}O
method. To get minimum usage for ecalj, need to read up to Section .5
...
\noindent To support of ecalj activity, we need your acknowledgment to \ecalj 
in your publications in the manner written in ecalj/README.md such as:\\
{\small\ \ \ \ \ 
...
T.Kotani, Hiori Kino, Mark van Schilfggarde,
...
which is on top of the Stuttgart's LMTO-ASA, mainly by van Schilfgarrde
...
(LDA/GGA/LDA+U, relaxiation of atomic positions), but also 
...
structure calculaitons \cite{xxx}.
...
\url{https://github.com/tkotani/ecalj} (or ecalj/README.md in the package).
...
many-body part (GW part) altenatively. The many-body part generate
...
but zero within MTs'. The IPW+''atomic like functions within MTs'' 
...
The Coulomb matrix $v$, the dynamically scened 
...
(but, in practical calculations, we need to reduce the size of the space
to reduce computational time).
We include full energy-dependence of $W$.
...
This is the reason whey we originally develope the QSGW method. 
The QSGW now becomes popular and taken as a possible candidate to go
beyond current limieation of such GW and LDA/GGA \cite{di_valentin_quasiparticle_2014}.
...
in the HSE funcional in addition}.
...
You may need to replace the extention \io{si} for your case.
(this extention is given by user. 
...
\begin{itemize}
\item LDA/GGA LDA+U, calculations, atomic forces and relaxiation.
Spin-orbit is included only for colinear spin-density cases.
...
we can use gfortran in Ubuntu 14.04 on Thinkpad T420s for a test purpose.
For productive runs, we may need multicores. 
...
(it is shown at \url{https://github.com/tkotani/ecalj}.)
We have a script \io{ecalj/InsallAll.*},
which install \ecalj\ and run a
series of tests automatically.
...
\item \verb#epsPP_lmfh_chipm#\\
      Non-interacting transverse spin polarization.
\item \verb#gw_lmfh#\\ One-shot GW calculation. 
       This also show life-time of QPs (\io{QPU\_lmf}).
\item \verb#genMLWF#\\ Wannier functions and matrix elements of $W$ on
...
\item Generate \ctrl from \ctrls by a script \verb+ctrlgenM1.py+,
     ...
     information to perform calculations. \io{ctrl.*} contains not only
      the content of \ctrls, but also other informations 
...
\item
     ...
     symmetry. In such a case, you have to ``standarize structure'' by
     VESTA or some other tools. This occures e.g., when we use a structure
...
\item Run LDA/GGA calculations. 
      ...
      (-MPIK meand kpoint parelell version).

\item Post processing.\\
      Plot energy band, DOS, PDOS, by running scripts.
      We can use scripts \exe{job\_band\_nspin1} or \exe{job\_band\_nspin2} for 
...
{\bf CAUTION!}: Positions of atoms are not necessarily fixed by \io{ctrl.*} when
...
when we run \verb#lmf# or \verb#lmf-MPIK#(k-parellel mpi version),
...
\begin{enumerate}
\item[Step 1.] 
Perform from the Step.1 thru the Step.3 in Sec.\ref{lda}
...
[Caution: We have to use the same \verb#LMXA# ($l$ in the expansion of eigenfunction 
...
\begin{figure}[h]
\includegraphics[width=10cm]{gwsc2014-12-091.ps}
...
which are requied for the ``main stage'' of GW calculaiton.
...
\fl{HAMindex} Hamiltonian index, all requied complex index for Hamiltonian of PMT method.
...
\item{\bf echo 0$|$hqpe\_sc}: gather datas and write down final results
...
\begin{verbatim}
  atom   l  nnvv  nnc !
    ...
    1    2    2    1
\end{verbatim}
`atom' means the atom number identifed in the {\bf ctrl} file.\\ 
...
\begin{verbatim}
  atom   l    n  occ unocc  ForX0 ForSxc ! Core (1=yes, 0=no)
    ...
    1    0    3    0    0      0    0    ! 3S
\end{verbatim}
Above options are about the product baiss within MT (Core).\\
...
We can generate Wannier functions (maximally localized Wannier
Functions) by a script \exe{genMLWF}.
Run examples in \io{ecalj/fpgw/MATERIALS/*MLWF}.
To run the script, we need to set options
in GWinput.
...
5 is the number of Wannier function. The most left one means $\phi$ index and the right one of it is $\dot\phi$ index. They are writen in the {\bf @MNLA\_CPHI} file.
...
\label{maininput}
\vspace{3mm}
\subsection{\bf generate a templete of {\sf GWinput}}
...
not directrly related to the difference among LAPW, LMTO, and PMT.


\item Eigenfunction within MTs (no parameters setting for them).\\
...
xxx does this really make the stabiization? xxx
This is the size of $\delta$ in denominator of $\Pi$ (EQ.xxx).
But (I think) we can (or can not) use it so as to make broadning 
...
Poles of  the Green fucntion $G^{\rm LDA}$ are treated as 
...
2: |q| is avarange in the Gamma cell (cell of BZ including Gamma point). \\
...
5:\ Use $\ds \delta_{nn'} \Sigma_{nn}(\epsilon_n)$ (Eigenvalue-only
      self-consistency, keeping the eigenfuncion as given)).
...
This overide setting in \verb#<QPNT>#. 
...
\item
     ...
     Need to repeat mkGWIN\_lmf2 to generate GWinput templete when you
...
\fl{ves*}
not meaninful at this stage

\fl{rhoMT*}
not meanigful at this stage
...
%------------------------------------------------------------------------
\newpage
\section{\exe{gwsc} script and its I/O Files}
\label{gwsc}
...
\noindent In the followins, We explain input/output files for each fortran program.
...
order to evaluate the weitht in the $\Gamma$ cell). See Eq.xxx in \cite{xxx}.
...
\fl{rst.si} (bin) Restart file of the lmf calculation. It contains all informations
...
\fl{PPOVLG.*} (bin) PPOVLGOverlap matrix xxxxxxxxxxoxf IPW. not exactly the the overlap matrix. see around line 500 in rdata4gw.m.f
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Check list for convergence on GW calculaitons}
\label{checklist}
Results could be dependent on cutoff parameters in \io{GWinput}.
In my opition, generally speaking, it is so easy 
...
may be required. But it dependes on case by case.
...
NOTE: Current PMT-QSGW method \cite{xxx} expand the static version of
      ...
      requied convergence). In such a case, we inevitably
      have to use empty spheres (MTO's) which is for empty region.

\item
Cores.\\
We usually use all cores as {\bf core1} (exchang only core). 
...
In prinicple, for larger n1n2n3, we can use smaller \verb#esmr#.
...
(on is default). If off, we use off-Gamma mesh (Gamma point is between
     mesh points) for dielectirc functions when we perform GW or QSGW.
``chi\_regqbz off'' may accelarate convergence on number of k points.
\end{itemize}
...
\begin{verbatim}
    ...
    -1     1     2     1    10     5
     0     1     2     1    11     5
\end{verbatim}
m is a magneitc quantum number, n is the dgree of freedom which means 1 : $\phi$, 2 : $\dot{\phi}$, and 3 : local orbital. l is the orbital angular quantum number. The match of orbitals and m number is shown by {\bf job\_pdos} command.The folloiwing number is the number of atom. And the next is the numerating number. It corresponds to the number in GWinput which is the most left one in the initial conditions.

\subsection{CPHI and GEIG}
We exapnd the eigenfunctions as the sum of the augmentation parts in MTs and the PW parts in the interstitial region:
\begin{displaymath}
\Psi_{{\bf k}n} = \sum_{{\bf R}u}\alpha_{{\bf R}u}^{{\bf k}n}\phi_{{\bf R}u}^{\bf k}({\bf r}) + \sum_{\bf G}\beta_{\bf G}^{{\bf k}n}P_{\bf G}^{\bf k}({\bf r})
\end{displaymath}
Here, {\bf CPHI} contains the information of $\alpha_{{\bf R}u}^{{\bf k}n}$ and {\bf GEIG} contains $\beta_{\bf G}^{{\bf k}n}$.
