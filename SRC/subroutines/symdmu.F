      subroutine symdmu(nlibu,dmatu,nbas,nsp,lmaxu,sspec,ssite,ng,g,!dmatw,
     .istab,lldau,rms)
      use m_struc_def  
...
C ... for spinor rotations
      logical cmdopt0
      real(8):: ddet33,xx
c      double precision eula(3),ddet33,xx
c      double complex u(2,2,ng)
      character *40 str
      dmatw=0d0
      rms = 0
      if (cmdopt0('--nosymdm') .or. ng .eq. 0) return
...
c        call dvset(eula,1,3,0d0)
c        call rm2eua(rmat,eula(1),eula(2),eula(3))
c        call rotspu(0,1,1,eula,1,u(1,1,ig))
