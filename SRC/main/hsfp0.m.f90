     ...
     &     alat,delta,deltaw,esmr_in=>esmr,clabl,iclass, ! diw,dw,
     &     il,in,im,nlnm, 
...
!    
c    For given ixc (read(5,*), hsfp0 yields variety of results.
c    (but some are already obsolate).
...
!! ixc=4,5,6 not checked now... NEED MPI and so on...
      elseif(ixc==4) then
        ...
        exchange=.false.
        tote=.false.
c        open(newunit=ifsecomg(1),file='SEComgU'//xt(nz))
c        if (nspin == 2) open(newunit=ifsecomg(2),file='SEComgD'//xt(nz))
...
c      call Rdpp(ngrp,symgg) =>moved to m_zmel
      call pshpr(60)
!! Readin WV.d
...
c          if(is==2) isx = iclose('XCD'//xt(nz))
        enddo
        endif !MPI__root
        deallocate(vxcfp)
      endif
c!!  Offset Gamma point Q0P
c      write(6,*) 'reading QOP'
c      ifiq0p=ifile_handle()
c      open (ifiq0p,file='Q0P')
c      read (ifiq0p,"(i5)") nq0i
...
c      nq0ix = nq0i
c      do i=1,nq0i
c        read (ifiq0p,* ) wqt(i),q0i(1:3,i)
c        if(wqt(i)==0d0 ) nq0ix = i-1
c      enddo
...
c        read(ifidmlx) wklm
c        close(ifidmlx) !jan2013 bugfix ifidmlx)
...
c     loop over spin ----------------------------------------------------
     ...
     i   nspin,is,
     i   qlat,ginv,qibz,qbz,wbz,nstbz, wibz, 
     i   nstar,irkip(is,:,:,:),  
     i   freq_r,freqx, wwx,   
     i   dwdummy, ecore(:,is),
