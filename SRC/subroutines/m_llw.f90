...
! this is dot(q(:)*w_mu(:,igb)). See PRB125102(2016) eq.(36)
 ...
 1115    continue               !iw
c         open(newunit=ifllw,file='LLWR.'//i2char(iq),form='unformatted')
c         write(ifllw) llw
c         close(ifllw)
      endif   
...
!! Full inversion to calculalte eps with LFC.
 ...
 1116   continue
c        open(newunit=ifllwi,file='LLWI.'//i2char(iq),form='unformatted')
c        write(ifllwi) llwi
c        close(ifllwi)
      endif
...
c----------------------------------------------------------
     ...
     &     MPI__Broadcast,MPI__DbleCOMPLEXsendQ,MPI__DbleCOMPLEXrecvQ, !MPI__rank,
     &     MPI__size,MPI__Qranktab,MPI__rankQ,MPI__rootQ
c !MPI__barrier, MPI__consoleout, !MPI__Ss,MPI__Se, MPI__allreducesumS,
c     &      !,MPI__rootS
!     ! === Recieve llw and llwI at node 0, where q=0(iq=1) is calculated. ===
      intent(in)::            iqxend
      integer:: iq0,dest,src,iq,iqxend
