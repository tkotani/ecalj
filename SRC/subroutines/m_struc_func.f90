...
c      integer:: lgunit
     ...
     , , master, MPI_COMM_WORLD,ierr)
c      i_data_size=size(struc%idxdn) ! idxdn(30)
c      call mpi_bcast(struc%idxdn, i_data_size,MPI_INTEGER
...
c      integer:: lgunit
      ...
      call MPI_COMM_RANK( MPI_COMM_WORLD, procid, ierr )
      call MPI_COMM_SIZE( MPI_COMM_WORLD, numprocs, ierr )
c      call mpi_bcast(struc%clabel, 1,MPI_REAL8 , master, MPI_COMM_WORLD,ierr)
...
c      i_data_size=size(struc%vel) ! vel(3)
c      call mpi_bcast(struc%vel, i_data_size,MPI_REAL8 , master, MPI_COMM_WORLD,ierr)
c       call mpi_bcast(struc%vshft, 1,MPI_REAL8 , master, MPI_COMM_WORLD,ierr)
      if (mlog) then
        call MPI_GET_PROCESSOR_NAME(name, resultlen, ierr)
        call strcop(shortname(procid),name,10,'.',ierr)
        namelen(procid) = ierr-1
