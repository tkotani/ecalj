...
**収束した `sigm.liti2o4` は同梱しない** (6³ で 13 MB、9³ で 30 MB。2026-09-17 に
リポジトリを軽くする方針で外した)。結果は上の QPU.*run・EFERMI・バンド図・
`bnd_*.tar.gz` に残してあり、再現するなら §5 の手順で `gwsc` を回す
(9³ は kt1 で 1 反復 4.3 h × 45 反復)。sigm の原本は kt1 の
`~/LiTi2O4/kbt/runs/n{666,999}_dq0.1_T*/` にある。
1000 K の 2 つは**全反復の生データも置いていない** (6³ で 17 MB、9³ で 26 MB
