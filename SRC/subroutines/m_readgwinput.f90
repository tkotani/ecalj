...
contains
  ...
  subroutine ReadGWinputKeys() ! Read values from GWinput
    ...
    if (have_toml) then
       ...
       if (allocated(errmsg) .and. len(errmsg) > 0) have_toml = .false.
    endif
    if (have_toml .and. gwinput_loaded) then
       ...
       ! ---- Legacy path: read from GWinput via getkeyvalue ----
       ...
       imbasd = -9999
       istat=-9999            ! istat=-9999 means number of readin arguments is returned in istat.
