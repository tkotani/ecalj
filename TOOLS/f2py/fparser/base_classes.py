...
class Variable:
    ...
    def analyze(self):
        ...
        if self.is_array():
            ...
            #print 'xxx',array_spec,' ::: ',self.is_deferred_shape_array()
            if self.is_deferred_shape_array(): # a(:,:)
                pass
            elif self.is_explicit_shape_array():
                shape = []
                for spec in array_spec:
                    #print  'lll',len(spec),' xxxyyy ',spec
                    if len(spec)==1:
                        shape.append(spec[0])
                    else:
                        #print 'zzzdx len2',spec[1],spec[0]
                        #shape.append(spec[1]-spec[0]) #takao correction. July2009
                        #shape.append(spec[1]+'-'+spec[0])
                        print 'zzzz',spec[1]+('-(%s)' % spec[0])
