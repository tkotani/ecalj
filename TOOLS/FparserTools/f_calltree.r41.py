...
#            print ins.__class__.__name__ #,ins.item.strline
#        continue
        loc1 =  ' in '+frame2+' '
        loc  =  ' '+ffile+(":L%d" % ins.item.span[0])
#        if(isinstance(ins, classes.Call)):      print "@cal Subr:"+ins.designator+loc1+loc #ins.item.strline
        if(isinstance(ins, classes.Type)):      print "@dcl Type:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
#        if(isinstance(ins, classes.Use)):       print "@use Modu:"+ins.name
         #label=ins.item.label
        if(isinstance(ins, classes.Program)):   print "@def Prog:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
        if(isinstance(ins, classes.Interface)) :print "@def ifc :"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap #,ins.a #,ins.a.variable_names
        if(isinstance(ins, classes.Subroutine)):
            print "@def Subr:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap #,ins.a #,ins.a.variable_names
            subs.append(ins.name)
            addfsdist(fsdict,ins.name,ffile)
        if(isinstance(ins, classes.Function)):  
            print "@def Func:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap #,type(ins) #,ins.a
...
# now all entry is classified to function (not correct but safer treatment)
        if(isinstance(ins, classes.Entry)):
            ...
            if(mother.__class__.__name__=='Subroutine'): 
                ...
                subs.append(ins.name)
                addfsdist(fsdict,ins.name,ffile)
            print aaa+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
        if(isinstance(ins, classes.Module)):        
            mods.append(ins.name)
            print "@def Modu:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
            addfsdist(fsdict,ins.name,ffile)
#########
...
#########
        if(isinstance(ins, block_statements.Type)): print "@def Type:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
        deptho=depth
        inso=ins
    ...
    for c in walk(parser.block):
        ...
        if(ps0.search(line)) :
            if(isinstance(ins, classes.Call)):      
                print "@cal Subr:"+ins.designator+loc1+loc+ ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
        ...
        if(isinstance(ins, classes.Use)):
            print "@use Modu:"+ins.name+loc1+loc + ' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap
            usedsf.add(ins.name)
...
#takao find that ins.item.line may not include full line data ---> it is replaced by F2PY_something in cases. Why???
#
        line= ins.item.line
        line= rr.sub("''",line)
...
#            print frame3
            ...
            if(mother != child):  #mother = child means a case where a function name is in the function.
                ...
                print "@cal Func:"+child+loc1+loc +' ---> '+lineall(ins.item)   #+ins.item.strline,ins.item.strlinemap  #,type(ins)
                dotdata.write( (mother+"->"+child+";\n").lower() )
        deptho=depth
        inso=ins
#print 'ppppp',line
