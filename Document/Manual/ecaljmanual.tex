...
calculation package, available freely.\\
...
ecalj/Document/Manual/). By \ecalj, we can do QSGW and related calculations
based on the PMT=LA\underline{P}W+L\underline{MT}O
method. To get minimum usage for ecalj, need to read up to Section .5
or Sec.\ref{cautionusage}. (We can generate model Hamiltonian through
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Theory and algolism for PMT-QSGW}
Explain this along the script fpgw/exec/gwsc.
After we did one-body self-consistent calculation, 
we do echo 0|lmfgw, resulting small files.
See Sec.\ref{gwsc}. Then we run qg4gw to generate q+G vectors
stored in QGpsi,QGcou,Q0P files.

\subsection{crystal structure}
We use unit \alat\ (a.u.) to represent length in the code.
Thus, in cases, to convert qunatities in the unit of a.u., we
multipy \alat to quantities.
For example, primitive cell is given by \alat\ (in a.u.) \times \plat.
That is, primitive vectors $\bfp_i$ (in a.u.) are
\verb#alat*plat(1:3,i)#, where i=1,2,3. (see \io{LATTC}).
The locations of MT sites $\{\bfR\}$ in the primitive cell
is given by \verb#alat*bas(1:3,ibas),ibas=1,nbas# 
(or \verb#pos# and \verb#natom# in cases). 
\verb#nbas# is the number of MT sites in the primitive cell.

\verb#getkeyvalue# is an universal i/o routine for GWinput.
Its arguments can take any time (logical, integer, real, (int or real) array).
Do grep 'call getkeyvalue' for fpgw/*/*.F to find out how to use it.

\begin{verbatim}
alat: unit in a.u. This is in LATTC or (or call genallc_v3)
plat: primitive vector. this is in LATTC (or call genallc_v3)
qlat: reciprocal primitive vector  \delta_ij= sum(plat(:,i)*qlat(:,j))
QpGcut_psi: cutoff to determine G vector for eigenfunction |q+G|< QpGcut_psi
QpGcut_cou: cutoff
symops (or symgg): space group, rotational part. This is in SYMOPS 
(or call genallc_v3)
nbas (or natom): number of MTs in the primitive cell.
bas(1:3,1:natom): MT centers R.
\end{verbatim}

mkqg.F is the main part of \exe{qg4gw} (fpgw/main/qg4gw.F).
iq0pin is hthe job control of mkqg. iq0pin=101 is only for backward compatibility. 
...
iq0pin=1: normal mode. Generate q and G for 
...
iq0pin mode:
generate q0p: algolizm
\subsection{eigenfunciton}
radial function
...
\end{displaymath}
Here, {\io CPHI} contains the information of $\alpha_{{\bf R}u}^{{\bf k}n}$ and {\io GEIG} contains $\beta_{\bf G}^{{\bf k}n}$.
