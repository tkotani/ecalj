...
! nlat for FFT of H(R+T i, R'j)
      ...
      enddo
c      print *,'m_gennlat_init: We got nlat and nqwgt'
...
!
c      allocate(nlatS(0:nk1-1,0:nk2-1,0:nk3-1,nbas,nbas))
c      allocate(nlatE(0:nk1-1,0:nk2-1,0:nk3-1,nbas,nbas))
      npair=0
      do ib1=1,nbas
        do ib2=1,nbas
          if(debug>0) print *
...
c$$$c check write
c$$$          if(debug>0) then
c$$$            write(6,"(a,2i4,2x,i6)") ' ib1 ib2 npair=',ib1,ib2,npair(ib1,ib2)
c$$$            do ni = 1,npair(ib1,ib2)
c$$$              posp =  pos(:,ib1)-pos(:,ib2) + matmul(plat,nlat(:,ni,ib1,ib2))
c$$$              rrr = sqrt(sum(posp**2))
c$$$              write(6,"(i6,3x,3i3,f8.3,i5)") ni,nlat(1:3,ni,ib1,ib2),rrr,nqwgt(ni,ib1,ib2)
c$$$            enddo
c$$$          endif  
        enddo
      enddo
