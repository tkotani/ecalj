...
c      do lm=1,(lx+1)**2
c        if(abs(funac(lm))>1d-6) write(6,"('  getwklm: lm funac =',i3,f18.9)") lm,funac(lm)
c      enddo
...
c        write(*,"(3f19.14)")epinv_(1,:,ik)
c        write(*,"(3f19.14)")epinv_(2,:,ik)
c        write(*,"(3f19.14)")epinv_(3,:,ik)
c        write(*,*)
      enddo
...
!!xxxxxxx NOTE:input q0i(1:3,1:nq0i) is kept. Output nq0i is larger than input nq0i.
      implicit none
      integer(4) :: ixx,ix,i,ngrp,ig,nq0i,nx06,ifix,irr(nx06),nqirr,nqibz,ib,nqbz
...
cwrite(6,"(a,3i3,3f10.5,' --> ',3f10.5)") ' q0=sym(ig)*q0i(:,ix) q0 ig q0i= ',i, ig, ix, q0i(:,ix), qt
cwrite(6,"(a,2i3,3f10.5)") ' q0(i)=sym(ig)*q0i(ix): ix ig q0= ',ix,ig, qt
cwrite(ifix) qt,q0i(:,ix),ix,ig !qtarget, q0i(ix),ig
                wt(ix) = wt(ix)+wt0(i)
                goto 990
              endif
            else
