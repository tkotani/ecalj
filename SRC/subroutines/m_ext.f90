...
999 continue
  ...
  subroutine print_usage_and_quit(prgnam)
    ...
    write(6,'(a)') '  --pr=N                console verbosity (overrides [io] verbose;'
    write(6,'(a)') '                        e.g. --pr=50 for tracebacks, --pr=70 for debug)'
    write(6,'(a)') '  --time=N[,M]          CPU timing log (overrides [io] time;'
    write(6,'(a)') '                        e.g. --time=5,5 for 5-level summary + on-the-fly)'
    ...
    write(6,'(a)') ''
    write(6,'(a)') 'Note: --pr / --time are the only cmdline shortcuts that override'
    write(6,'(a)') '      a TOML key directly (legacy dual-form, predating -v[<path>]=).'
    write(6,'(a)') '      For any other TOML key, use -v[<dotted.path>]=<value>.'
    write(6,'(a)') ''
    flush(6)
