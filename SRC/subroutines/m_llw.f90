...
!    complex(8):: zxq(nmbas1,nmbas2,nw_i:nw)
    ...
    !$acc enter data create(zxqw, epstilde, zw, x_m2e) copyin(vcousq)
    if(nspin == 1) then
    ...
    if(iq<=nqibz) then        !for mmmw
      ...
      iwloop: do 1015 iw  = nwmin,nwmax
        ...
        rootq_if1:if(mpi__root_q) then
          ...
          ! W^[M] = AW^[E]A^\dagger
          EtoMBasisTransformation: if(is_m_basis .and. iq /= 1) then
            call stopwatch_start(t_sw_x_m2e_xf)
...
1115  enddo
    ...
    !$acc exit data delete(zxqw, epstilde, zw, vcousq, x_m2e)
    deallocate(zxqw)
    if(mpi__root_q) then 
...
!    complex(8):: zxqi(nmbas1,nmbas2,niw)
    ...
    type(stopwatch) :: t_sw_matinv, t_sw_x_gather, t_sw_x_m2e_xf
    allocate (zxqw(ngb, ngb), x_m2e(ngb,ngb))
    !$acc enter data create(zxqw, x_m2e, epstilde, zw) copyin(vcousq)
    mreclx=mrecl
    emptyrun=.false. !cmdopt0('--emptyrun')
    if(init) then
