...
echo "$space rm -f {atm,eula,moms,mixm,rst,rsta,save,log,hssn,wkp,cv,bsmv,syml,bnds,dmats,dmats-save,sigm,qpp}.$ext $testout"
             rm -f {atm,eula,moms,mixm,rst,rsta,save,log,hssn,wkp,cv,bsmv,syml,bnds,dmats,dmats-save,sigm,qpp}.$ext $testout
if (! $?clean) then
echo "$space cp $cplst ."
             cp $cplst .
endif
