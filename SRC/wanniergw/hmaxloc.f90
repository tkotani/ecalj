...
c     use m_readqg,only: readngmx,readqg
     ...
     &     nqbz,nqibz,nqbzw,nteti,ntetf,n1,n2,n3,qbas=>qlat,ginv, !qbasmc,
...
c         write(6,"('elist: q iq is nwfi nwfe e(nwf)= ',3f9.4,i5,i2,2i5,f10.3)") qxx,iq,is,inii,nwf+inii-1, eeee
...
c      do iq=1,nqbz
c      write(6,"('iiii: iq and internal window region=',3i5)")iq,iki_i(iq),iki_f(iq)
c      enddo
...
c <r_n> ([1] eq.31)
     ...
     o               rn)
c         do i=1,nwf
c            write(6,ftox)'wwww: i,i, <wannier i|r|wannier i> ',i,ftof(rn(1:3,i))
c         enddo
         do iq = 1,nqbz
            dwmn = (0d0,0d0)
            do ibb = 1,nbb
...
c     (2-6) Omeg_I, Omega_D and Omega_OD ([1] eq.34,35,36)
     ...
     d               nwf,nqbz,nbb,
     o               omgi,omgd,omgod,omgdod,omgidod,alat,iprint)
...
c output
     ...
     i             qbz,umnk,cnk,eunk,
     i             iko_ix,iko_fx,iko_i,iko_f,
     d             nwf,nqbz,nband,nlmto, is,dnk)
c$$$cccccccccccccccccccccccccccccccccccccccccccccccccc
c$$$      write(6,ftox)' uuuu'
c$$$      do iq=1,nqbz
c$$$      do iwf=1,nwf
c$$$c         write(6,ftox)' uuuu umnk  ',is,iq,ftof(umnk(iwf,1:nwf,iq),2)
c$$$c         write(6,ftox)' uuuu   amnk',is,iq,ftof(amnk(iko_ix:iko_fx,iwf,iq),2)
c$$$         do iwf2=1,nwf
c$$$            cccx(iwf2)= sum( dconjg(amnk(iko_ix:iko_fx,iwf,iq))*cnk(iko_ix:iko_fx,iwf2,iq) )
c$$$         enddo
c$$$         write(6,ftox)' uuuu   <G|psi*c  >',is,iq,ftof(cccx,3)
c$$$         write(6,ftox)' uuuu   <G|psi*c*u>',is,iq,ftof(matmul(cccx,umnk(:,:,iq)),3)
c$$$         do iwf2=1,nwf
c$$$            cccx(iwf2)= sum( dconjg(amnk(iko_ix:iko_fx,iwf,iq))*dnk(iko_ix:iko_fx,iwf2,iq))
c$$$         enddo
c$$$         write(6,ftox)' uuuu   <G|ib>dnk',is,iq,ftof(cccx,3)
c$$$      enddo
c$$$      enddo
c$$$cccccccccccccccccccccccccccccccccccccccccccccccccc
      deallocate(dnk)
...
!! ----------------------------------------------------------
!! k-point mesh
c      call get_nqbze(nqbz,nqbze)
c      allocate(qbze(3,nqbze))
c      call get_qbze(qbz,nqbz,
c     o              qbze,nqbze)
