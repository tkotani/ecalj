...
contains
  subroutine momusl(z,rmt,lmxa,pnu,pnz,rsml,ehl,lmxl,nlml,a,nr,nsp,rofi,rwgt,v0,v1,qum,vum)!Moments of ul*ul,ul*sl,sl*sl and their integrals with true pot.
    ...
    !l   sl    :radial wave functions;  
    !l   ruu   :diagonal w.f. products; 
    !l   rus   :diagonal w.f. products; 
    !l   rss   :diagonal w.f. products; 
    !r Remarks
    !r   Diagonal integrals (l1=l2, l=0 potential) are treated specially:
    !r   small component of w.f. is explicitly taken into account.
    implicit none
    integer :: lmxa,lmxl,nlml,nr,nsp,l1,l2,lm,i,mlm,isp
    ...
    Moments: do concurrent(isp=1:nsp, l1=0:lmxa, l2=0:lmxa,lm=0:lmxl)
    ...
    enddo Moments
    Integrals :do concurrent(isp = 1:nsp, mlm = 1:nlml)  ! --- ul*ul*V_M, ul*sl*V_M, sl*sl*V_M ---
  ...
  subroutine vlm2us(lmaxu,rmt,idu,lmxa,iblu,vorb,phzdphz,rotp,vumm)!- Rotate vorb from (phi,phidot) to (u,s) and store in vumm
    ...
    lloop: do l = 0, lmaxu
       ...
       iblu = iblu+1
       do  i = 1, 2
          rotpp  = rotp(l,i,:,:) !Rotate Vorb from phi,phidot basis to u,s basis
          ...
          vumm(:,:,1,1,i,l) = Vorb(:,:,i,iblu)*rotpp(1,1)*rotppt(1,1)
          vumm(:,:,1,2,i,l) = Vorb(:,:,i,iblu)*rotpp(1,1)*rotppt(1,2)
          vumm(:,:,2,1,i,l) = Vorb(:,:,i,iblu)*rotpp(2,1)*rotppt(1,1)
          vumm(:,:,2,2,i,l) = Vorb(:,:,i,iblu)*rotpp(2,1)*rotppt(1,2)
          phz  = phzdphz(1,l+1,i)
