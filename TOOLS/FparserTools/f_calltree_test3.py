...
#    print '---- '+ffile+' start -----'
    ...
    for c in walk(parser.block):
        ...
        classname=ins.__class__.__name__
        if( classname=="Line") :
            item=ins
        else:
            item = ins.item
...
# This is a map for 'If' Class.
#
        if(classname=='If'):
            sline=item.strline
            mapo=item.strlinemap
            print 'xxxxxxxx',sline
            axx=ppp.search(sline)
            print 'xxxxxx2 ',axx.group()
            #print axx.group()
            pare = rrr.search(axx.group())
            print 'yyyyyy ',pare.groups()
            sline=axx.group()
            print sline
            print mapo

        print len(sstack),depth, ins.__class__.__name__, item.span,  sline, item.label , item.strlinemap
        deptho=depth
        inso=ins
        continue
