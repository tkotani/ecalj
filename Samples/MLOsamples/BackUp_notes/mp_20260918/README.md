...
| Ni, 古い `__mixm` 無しで 0.66 μB | mp-23 | 強磁性 | 0 | 20 / 148 (↑), 20 / 134 (↓) | 19 / 136, 16 / 134 |
| Fe, 同 2.24 μB | mp-13 | 強磁性 | 3 (Fe 3p LO) | 17 / 218 (↑), 18 / 147 (↓) | 19 / 218, 20 / 147 |
...
3. **Ni・Fe が非磁性に落ちた — 真因は古い混合履歴 `__mixm.<sname>` の継承**（当初は
   `mix = "B3", b = 0.2` のせいと誤診したが、b の走査で 0.2 だけ潰れたのは古い `__mixm`
   を消し忘れた 1 本だけだった）。非磁性に収束した run の履歴で Broyden の最初の更新を
   作ると非磁性の不動点へ飛ぶ。`__mixm` を消せば B3/0.2 で 2.24 μB（16 反復）。
   修正: rst 無しの原子密度スタートでは lmf が `__mixm` を捨てる、`ctrlgenToml` が内部
   run の `__mixm` を残さない、レコード長が違う `__mixm` は無視。テンプレの既定は
   B3/0.2 のまま。Fe / Ni の MLO は上の表のとおり両スピン 17〜20 meV。
