...
# print "->",mpi,program,echoinput,target,output
  print "#>>>"
  if (length(echoinput)>0) { cmd=cms sprintf("echo %s > %s\n", echoinput,tempfile)}
  print cmd
  cmd=""
  if (mpi>0) { cmd = cmd sprintf("%s -np $MPI_SIZE ",mpirun) } 

  if (program=="hx0fp0_sc") { runoption="-nq 2 -nm 2" } # for -np 4 , q=2,m=2 parallel  
  else { runoption="" }

  if (length(program)>0) { cmd= cmd sprintf("%s %s ",programdir program,runoption); }
  if (length(target)>0) { cmd = cmd sprintf("%s ",target); }
  if (length(target2)>0) { cmd = cmd sprintf("%s ",target2); }
  if (length(target3)>0) { cmd = cmd sprintf("%s ",target3); }
  if (length(echoinput)>0) { cmd = cmd sprintf("< %s ",tempfile) }
  if (length(output)>0) { cmd = cmd sprintf("> %s ",output); }
  print "echo \"         "cmd"\" "
  print cmd
