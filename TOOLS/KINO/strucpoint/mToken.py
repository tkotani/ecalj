...
class t_var:
	...
	tok=[]
	array=[]
	def __init__(self,name,element,tok,array):
		self.name=name
		self.element=element
		self.tok=tok
		self.array=array
	def __str__(self):
		return self.name +","+ self.element +","+ list2string(self.tok) + ","+list2string(self.array)
		

def separate_var(tok):
	thisfunc="separate_var"
	if isinstance(tok,list)==0:
		print thisfunc,"ERROR, tok must be list"
		sys.exit(10)
	l_tok=tok
	l_tok=do_split0__ils_ol(l_tok,'(')
	l_tok=do_split0__ils_ol(l_tok,')')
	l_tok=do_split0__ils_ol(l_tok,',')
	l_tok=do_split0__ils_ol(l_tok,'%')
	var=l_tok[0]
	...
	for nn in l_tok:
		if nn=='%':
			ifound=ifound+1
	if ifound>1:
		print thisfunc,"ERROR, more than one %, tok=",tok
		sys.exit(10)

	pos_p=-1
	for i in range(len(l_tok)):
		nn=l_tok[i]
		if nn =='%' and i+1<len(l_tok):
			pos_p=i
			element=l_tok[i+1]
			break
	iarray=[0,0]
	if pos_p >0:
		for i in range(pos_p):
			if l_tok[i]=='(':
				iarray[0]=i
				break
		for i in range(pos_p+1,len(l_tok)):
			if l_tok[i]=='(':
				iarray[1]=i
				break
	elif pos_p==-1:
		...
		print thisfunc,"ERROR strange token=",tok
		sys.exit(10)

	return  t_var(var,element,l_tok,iarray)
...
def add_list(list1,list2):
	listt=[]
	lists=[]
	if isinstance(list1,list)==1:
		listt=list1	
	else:
		listt=[list1]
	if isinstance(list2,list)==1:
                lists=list2
        else:
                lists=[list2]
	for nn in lists:
...
def list2str(tok,p=' '):
	s=''
	if isinstance(tok,basestring)==1:
		return tok
