...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    write(ifi,'(a)') '# tetrakbt   = true   # chi0 with Fermi-Dirac occupations at t_tetrakbt (finite-T tetrahedron,'
    write(ifi,'(a)') '#                     # method B-prime) and the finite-T Fermi level EFERMI_kbt; false = T=0 tetrahedron'
    write(ifi,'(a)') '# t_tetrakbt = 300    # (K) electronic temperature for chi0'
    write(ifi,'(a)') '# t_sigmakbt = 300    # (K) Fermi-Dirac occupations in Sigma too; set equal to t_tetrakbt (needs tetrakbt)'
