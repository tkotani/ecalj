...
−7 eV より上は両者同程度なので、荒れは O 2p の底だけ。バンド補間ではなく Σ そのもの:
Γ のメッシュ点で state 9 の SEc が 6³ 5.96 → 9³ 4.50 eV (Σ−vxc +0.23 → −1.39 eV)、
6³ で三重縮退の state 10–12 が 9³ では 0.45 eV 割れる (立方対称の破れ)。O 2s (−21 eV,
state 7,8) は一致。offset-Γ (`deltaq_scale` = 0.1 は 9³ の間隔 1/9 に近い) か有限温度 Σ の
実軸ビニング (`esmr` 窓) が深い O 2p で破綻している疑い — と思ったが、反復ごとの推移を
見ると **1 反復目は 9³ も 6³ と同じく滑らか (band 9: 9 meV) で、反復で育つ**
(3 反復 14、10 反復 20、20 反復 112、45 反復 82 meV; 6³ は 30 反復まで 7〜9 meV)。
一発の Σ の精度ではなく、9³ × 有限温度での QSGW 反復の不安定性 (Γ 近傍の深い状態への
Σ の帰還) で、E_F 近傍の収束判定には掛からず居座る。有限温度が絡むかは 9³ の T=0 の
反復が無いので不明 (未実施: 9³ T=0 を 10 反復、または `scaledsigma=0.8`)。**§3.3 の「メッシュ依存が消えている」は E_F ±0.5 eV の話で、O 2p の底には
当てはまらない。**
