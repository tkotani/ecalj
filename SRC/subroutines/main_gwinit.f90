!> Generate the GW-related part of ctrlG.<sname>.toml + PB.toml.
...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    write(ifi,'(a)')
    write(ifi,'(a)') '# Per-atom product-basis tables (nlx / valence / core) live in PB.toml'
    write(ifi,'(a)')
    ...
    !! ===== Write PB.toml (per-atom product-basis tables) =====
    open(newunit=ifpb, file=pb_partial)
    write(ifpb,'(a)') '# PB.toml -- per-atom product basis tables.'
    ...
    close(ifpb)
    call execute_command_line('mv '//pb_partial//' PB.toml', wait=.true.)
