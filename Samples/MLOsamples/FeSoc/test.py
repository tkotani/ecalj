...
def test(args, bindir, testdir, workdir):
    ...
    # --- Test 1: non-SOC MLO bands (nspin=2 Fe ferromagnetic) ---
    cleanup()
    runprogs([f'{job_mlo} fe {npflag} --NoGnuplot'])
    ...
    # --- Test 2: SOC MLO bands via job_mlo_soc (2N spinor in spin1 file) ---
    cleanup()
    runprogs([f'{job_mlo_soc} fe {npflag} --NoGnuplot'])
    ...
    print()
    print('=' * 70)
    print(f'To view the SOC band plot (last run is job_mlo_soc, 2N spinor):')
    print(f'  cd {workdir}')
