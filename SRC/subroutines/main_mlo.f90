...
module m_mlo
  ...
  private
contains
  subroutine mlo() ! Get the Hamiltoniand on the MT-Projected orbitals <MPO|H|MPO> and overlap <MPO|MPO>
  use mpi
  use m_HamPMT,only: ReadHamPMTInfo, HamPMTtoHamRsMLO, plat,npair,nlat,nqwgt,ldim,nkp,qplist,ib_table,alat,symops,ngrp,pos,nspc
  !                                  HamPMTtoHamRsMLO do not change variables here. Only generate HamRsMTO file.
  ! use m_HamRsMLO,  only: ReadHamRsMLO, hammr,ovlmr,ndimMTO, npairmx,nspx,ib_tableM
  use m_mlo_ham, only: read_ham_rs, calc_ham_eigen, nspx, ndimMTO, npairmx, socmatrix
...
!  use m_prgnam,only: set_prgnam
  ...
  character(256):: fband(2)=['band_MLO_spin1.dat','band_MLO_spin2.dat'],cccx
  character(8):: prgnam=''
!  include "mpif.h"
...
!  if(master_mpi) write(stdo,ftox)'Read qplist.dat: ndat =',ndat
  ...
  !        Spinloop: do jsp=1,nspx !nsp is the number of spin.  When lso=1(Lz.Sz), nspx=1
          ...
          !         hamm(i,j)= hamm(i,j)+ hammr(i,j,it,jsp)*phase !MLO Hamiltonian at qp
       ...
       do ikp=1,ndatx
          if(socmatrix) then !2N spinor eigenvalues written to spin1 file
             ...
             do jsp=1,nspx
                ...
                if(jsp==2) ifsy=ifsy2
                do i=1, ndimMTO
                   write(ifsy,"(f15.5,f15.5,2i4)") xdat(ikp),evl(i,jsp,ikp),jsp,i !for gnuplot
                enddo
             enddo
          endif
       enddo
...
!          'OK! Run gnuplot -p '//trim(fname1)//'. Brown points are by HamRsMTO for Hamiltonian on {|MLO1>}'
  ...
  call rx0('OK! end of mlo. Get MLO Hamiltoian.')
end subroutine mlo
