...
損失関数の候補(重みは「この誤差で 1」と読める形):

$$
L=\Big(\frac{\Delta E_{\rm gap}}{10\,{\rm meV}}\Big)^2
+\Big(\frac{\ln(m^*_{\rm MLO}/m^*_{\rm DFT})}{0.05}\Big)^2
+\Big(\frac{\Delta v/v}{0.05}\Big)^2
+\Big(\frac{\Delta k_F}{0.005}\Big)^2
+\Big(\frac{{\rm std}_{dx}[m^*]/\overline{m^*}}{0.05}\Big)^2
$$

第 3・4 項(速度と $k_F$)が今回追加した観点、第 5 項は「$m^*$ のフィット窓依存性」で
バンドの滑らかさを直接測る。どの項をどの重みで入れるかは**まだ決めていない** —
用途(輸送係数なら速度、光学なら遷移エネルギー)で変わるため。
