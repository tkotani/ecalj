...
C ... Local parameters
     ...
     .force(3),plat0(3,3),qlat0(3,3), !,vel(3)
...
! MPI check to see if at least 1st record can be read. Abort with error message if file is missing (lfail = .true.)
 ...
 996       continue
        ...
        if (procid .eq. master) then
            ...
            read(jfi) nit
            read(jfi) alat0,vol0
            if (ipr .ge. 40) write(stdo,710) trim(fid0),trim(usernm),trim(hostnm),trim(datimp)
...
C   --- Read information related to dynamics ---
        if (procid .eq. master) read(jfi) wk 
        ...
        do ib = 1, nbas0
          if (procid .eq. master) read(jfi,err=999,end=999) jb,pos,force
          call mpibc1_real(pos,  3,'iors_pos')
          call mpibc1_real(force,3,'iors_force')
          if(ib > nbas) cycle
          if(irs3 .ne. 0) pos=ssite(ib)%pos
          ssite(ib)%pos  =pos
          ssite(ib)%pos0 =pos
...
c            ltmp2 = .false.
c            do  i = 1, lmxa+1
c              vec0(1) = mod(pnz(i,1),10d0)
c              if (vec0(1) .eq. 0) vec0(1) = pnu(i,1)
c              vec0(2) = mod(pnzs(i,1),10d0)
c              if (vec0(2) .eq. 0) vec0(2) = pnus(i,1)
c              ltmp1 = ltmp1 .or. int(pnu(i,1)) .ne. int(pnus(i,1))
c              ltmp2 = ltmp2 .or.
c     .        int(mod(pnz(i,1),10d0)) .ne. int(mod(pnzs(i,1),10d0))
c              lfail = lfail .or. min(int(pnu(i,1)),int(vec0(1))) .ne.
c     .        min(int(pnus(i,1)),int(vec0(2)))
c            enddo
            if (ipr.ge.20) write(stdo,203) ib,spid,'file pnu',(pnu(i,1), i=1,lmxa+1)
c            if (ltmp1 .and. ipr.ge.20) write(stdo,204) 'given pnu is',(pnus(i,1), i=1,lmxa+1)
            if (ipr.ge.20) write(stdo,203) ib,spid,'file pz ',(pnz(i,1), i=1,lmxa+1)
c            if (ltmp2 .and. ipr.ge.20) write(stdo,204) 'given pz  is',(pnzs(i,1), i=1,lmxa+1)
  ...
  204       format(26x,a,8f6.2)
            ...
            if (procid .eq. master) then
              ...
              if (kmax0.ne.kmax.and.ipr.ge.10) write(stdo,201) ib,spid,'kmax',kmax0,kmax
              if (lmxa0.ne.lmxa.and.ipr.ge.10) write(stdo,201) ib,spid,'lmax',lmxa0,lmxa
...
!!         
          ...
          if (procid .eq. master) then
c             print *,'nnnnnnnnread',ib,nr*nlml*nsp,nr,nlml,nsp
...
C     ... For now, ASA stores no core data
  ...
  364 format(/9x,'ib:spc la ll   rmt     nr   a     pnu')
      ...
      return
 998  continue
      if (ipr .gt. 0) write(stdo,'('' iors  : empty file ... nothing read'')')
      close(ifi)
      return
  999 continue
