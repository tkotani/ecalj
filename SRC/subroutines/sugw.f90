...
c      use m_shortn3,only: shortn3_initialize,shortn3
      use m_mkpot,only: 
...
c      integer,allocatable:: igv2(:,:)
      ...
      integer(4)::   irr,nqirr, nmcore !feb2012takao nk1,nk2,nk3, 
...
!     !         Write orthonormalized valence wave functions for this atom
            do  l = 0, lmaxa
               do  i = 1, nsp
                  ...
                  if (konfig(l) .ge. 10.and.master_mpi) write(ifigwa) gval(1:nr,1,l,3,i)
               enddo
            enddo
...
c        if (dipolematrix.and.iq>nqbz) exit
...
c        if(dipolematrix) allocate(dipo(ndimh,ndimh,3))
        ispSS=1
        ispEE=nsp
...
c! note. This was intended for dipole but wrong since x,y,z are not periodic
c          if(dipolematrix)then  !From spotd,and ppixd, dipo(=x,y,z)=<F_i|x,y,z|F_j> . Experimental.
c            write(stdo,'("dipole matrix calculated")')    !is this is ambiguous for x+dx shifts but good for wannier?
c            call hambl(isp,qp,spotxd(:,:,:,:,1),vconst,sv_p_osig,sv_p_otau,sv_p_oppixd(:,:,1),dipo(:,:,1),ovl)
c            call hambl(isp,qp,spotxd(:,:,:,:,2),vconst,sv_p_osig,sv_p_otau,sv_p_oppixd(:,:,2),dipo(:,:,2),ovl)
c            call hambl(isp,qp,spotxd(:,:,:,:,3),vconst,sv_p_osig,sv_p_otau,sv_p_oppixd(:,:,3),dipo(:,:,3),ovl)
c          endif   
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c          qp  = qplist(:,iq)-[1d-6,2d-6,3d-6] !A trick to shift qp to avoid ambiguity of degeneracy
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c          if(dipolematrix) then 
c             dipo(:,:,1)= dipo(:,:,1)-vxc ! dipo(i,j,1) = <Fi|x|Fj>
c             dipo(:,:,2)= dipo(:,:,2)-vxc ! dipo(i,j,2) = <Fi|y|Fj>
c             dipo(:,:,3)= dipo(:,:,3)-vxc ! dipo(i,j,3) = <Fi|z|Fj>
c          endif   
          vxc = ham - vxc ! vxc(LDA) part 
          if (lwvxc) write(ifiv) vxc
c          if (dipolematrix) write(ifiv) dipo,ovl
...
c             if(sum(abs(qp-q))>1d-8) stop 'sugw:qp/=q; qp=p sep2012'
             inn=0
...
c          write(ifigw1) qp, (evl(i,isp),i=1,ndimh)
...
c             call zgemm('C','N',ndimh,ndimh,ngp,(1d0,0d0),pzovl,ngp,pwz,ngp,(0d0,0d0),testc,ndimh)
             deallocate(pzovl)
...
c             if (abs(sum(q-qp)) .gt. 1d-10) then
c                write(ifinormchk,555) iq,qp,qp
c             else
...
c          call zgemm('C','N',ndimh,ndimh,ndimh,(1d0,0d0),evec,ndimh,vxc,ndimh,(0d0,0d0),testc,ndimh)
...
c       if(allocated(dipo)) deallocate(dipo)
...
c$$$      subroutine ioaindx(npqn,lmxax,nbas,ndima,nlindx)
c$$$C-  read NlAindx
c$$$C ----------------------------------------------------------------------
c$$$Ci Inputs
c$$$Ci   npqn  :leading dimension of nlindx
c$$$Ci   lmxax :second dimension of nlindx
c$$$Ci   nbas  :size of basis
c$$$Ci   ndima :number of augmentation channels
c$$$Ci   nlindx:pointer to augmentation channels by site
c$$$      implicit none
c$$$      integer npqn,lmxax,nbas,ndima,ifi,nlindx(npqn,0:lmxax,nbas)
c$$$      character outs*80
c$$$      integer i,ipqn,l,ib,ii
c$$$      open(newunit=ifi,file='NLAindx')
c$$$      nlindx = -1
c$$$      read(ifi,'(a)') outs
c$$$      read(ifi,*) i
c$$$      if (ndima .gt. 0 .and. i .ne. ndima)
c$$$     .  call rx('ioaindx: file mismatch NLAindx')
c$$$      ndima = i
c$$$      do  i = 1, ndima
c$$$          read(ifi,'(a)',err=101,end=101) outs
c$$$          read(outs,*) ipqn,l,ib,ii
c$$$          nlindx(ipqn,l,ib) = ii
c$$$       enddo
c$$$ 101   continue
c$$$      close(ifi)
c$$$      end subroutine ioaindx
