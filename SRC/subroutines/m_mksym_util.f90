...
contains
  subroutine gensym(slabl,gens,symfind,nbas,nspec,ngmx,plat,platcv,bas,ips, nrspec,ng,g,ag,ngen,gen,nwgens,nggen,isym,istab) !Generate the space group ops
    ...
    !i  nbas:  On input, number of atoms in the basis
    !i  bas:   basis vectors
  ...
  subroutine sgroup(gen,agen,ngen,g,ag,ng,ngmx,qb)  !- Sets up space group ops for given generators.
    !i Inputs
...
99  continue
  ...
  subroutine parsop(t,i,a)    !- Parse string for a point group operator
    ...
    if (t(i:i) == 'r' .OR. t(i:i) == 'R') then
       ...
       a(3,1) = a(3,1) - s*v(2)
       a(2,3) = a(2,3) - s*v(1)
    elseif (t(i:i) == 'm' .OR. t(i:i) == 'M') then
       i = i+1
       if( .NOT. parsvc(-1,t,i,v)) goto 99
       sp = sum(v**2)
       do  j = 1, 3
          ...
          a(j,j) = a(j,j) + 1d0
       enddo
    elseif (t(i:i) == 'i' .OR. t(i:i) == 'I') then
       i = i+1
       a=-reshape(e,[3,3])
    elseif (t(i:i) == 'e' .OR. t(i:i) == 'E') then
       i = i+1
       a= reshape(e,[3,3])
    else
       goto 99
...
60     enddo
  ...
  subroutine csymop(iopt,grp,li,nrot,vecg) !Decomposes a group operation into its consitutents, or vice-versa
    ...
    implicit none
    integer :: nrot,iopt
    real(8) :: vecg(3),grp(3,3)
    logical :: li
    integer :: i,idamax,j,in
    real(8) :: costbn,detop,ddet33,dnrm2,sinpb3,twopi,vfac, wk(9),sintbn,omcos,ddot
    character(8):: xt
    twopi = 8*datan(1d0)
  ...
  subroutine symtbl(mode,nbas,pos,g,ag,ng,qlat,istab) ! Make symmetry transformation table for posis atoms; check classes
    ...
    !i   toll   :tol for which atoms are considered to be at the same site
    !o Outputs
