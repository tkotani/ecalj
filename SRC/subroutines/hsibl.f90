...
contains
  ...
  subroutine hsibl(k1,k2,k3,vsm,isp,q,ndimh,napw,igapw, h)
    ...
    call gvlst2(alat, plat, q, n1, n2, n3, gmin, gmax, [0], 509, ng, ng, kv, gvv, iv)
    call poppr
    ...
    ib1loop: do  iloop = ibini,ibend
      ...
      irob1loop: do  iorb1 = 1, norb1 !MTO orbital loop
        ...
        do  ilm = nlm1, nlm2
          w_oc1(1:ng,ilm+offi) = cwork(1:ng)* yl(1:ng,ilm) !PW coefficients for MTO(ilm+offi)
...
333 continue
    if(napw==0) goto 666
    qgVqg: block ! --- <e^i qpG | V |e^i qpG'>/vol = V(G'-G) ---
