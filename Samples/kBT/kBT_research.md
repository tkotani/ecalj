...
極項の窓は st 45 で 3 eV → 窓の中にプラズモン（1.8 eV）があり、**核の幅（kBT ≈ 0.09 eV、
FWHM 0.3 eV）で均しても極の位置依存が残る**（極幅 0.1 eV と同程度でしかない）。一発目で
消えたのは、たまたま極の当たり方が良かった側面がある。
→ wcsmear の核幅は「重みの核と同じ」という原理で決めたが、極の均しには不足。選択肢:
(a) 極項専用の幅（`wcsmear_width`、例 0.3〜0.5 eV）を別に持つ、(b) W 側で SmearX0 を併用、
(c) 四面体版。iter 3 の結果（07:00 ごろ）を見てから判断。
