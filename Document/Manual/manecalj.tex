...
from the PMT part to the GW part. 
The eigenfuncions are expanded by the two types of basis functions,
...
but zero within MTs'. The IPW+''atomic like funcitons within MTs'' 
make 'a basis set to expand eigenfuncitons'.

We need another basis set to expand ``product of eigenfuncions''. 
That is, the mixed product bases (MPB) \cite{xxx}, 
...
solutions to the Schr\"odinger equation within the MT sphere \cite{xxx}.
...
in the MPB.
It can virtually span all the space made of product of eigenfunctions
(but, in practical calculaitons, we need to reduce the size of the space
...
Spin-orbit can be included not for non-colinear cases.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Package installation, tests and samples.}
See \io{ecalj/README} and Getstarted.pdf.
At \io{ecalj/} directory, run \io{/Install.ifort}.
Note that it copies all requied binaries and scripts
to your \io{bin/} directory specified in the script 
(\verb#BINDIR=# in specify to which you will copy).
You may need to modify Math library (blas,lapack fftw), 
and compile options in Install.ifort.
...
%\begin{itembox}[l]{Steps to do LDA/GGA calcualtions with ecalj}
\begin{enumerate}
...
\item{\bf rdata4gw\_V2}: 
  Read {\sf DATA4GW\_V2} and so on, and decompose it into files required
     in the followings. And calculate PPOVL* files.
...
\item{\bf echo 3$|$hsfp0\_sc  }: gives the Core exchange part of the self-energy.
...
for each $l$ channel. (dec2014:\verb#<PBASMAX># is not checked recently;
