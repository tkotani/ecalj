...
Ci                rs3 eh3 vmtz
Ci     Stored:    name coreh z rmt a nr norp ntorb 
Co Outputs via iofa
Cl Local variables
...
Cu   22 Dec 01 Adjustments to accomodate changes in phidxasz
...
Ci         :xxx nrmix(2) = no prior iterations Anderson mixing in
...
!! plplus, qlplus are higher ones (when PZ exist).
      plplus=0d0
      qlplus=0d0
c      print *,'NOTE: when we have two valence: P and PZ, We assume eigen(PZ) is deeper than eigen(P).'
      do  101  i = 1, nsp
      do  10  l = 0, lmxa
        pnul = pnu(l+1,i)
...
!!       jun2012 feb2011
        if (pzl .ne. 0) then
           if(int(pnul-1) .eq. int(pzl)) then !pz is deeper than pnul
...
!! pl,ql are for PZ when PZ= nonzero exist... somehow confusing.
        ...
        ql(1,l+1,i) = qval
        if (nsp .eq. 2.and. pnul .eq. 0) pl(l+1,i) = pl(l+1,1)
        ql(2,l+1,i) = 0d0
        ql(3,l+1,i) = 0d0
...
c      write(stdo,"(a)")'conf When int(P)z .ne. int(P), Qval: Q for MTOcore(PZ)+MTO(P)'
...
c      call dcopy(lmxa+1,ql,3,qat,1)
c      call dcopy(lmxa+1,ql(1,1,nsp),3,qat(1,2),1)
c      call awrit4('fa  Pl %n:-1d  Ql %n:-1d',
c     .' ',80,stdl,lmxa+1,pl,lmxa+1,qat)
c      if (nsp .eq. 2)
c     .call awrit4('fa  Pl2%n:-1d  Ql2%n:-1d',
c     .' ',80,stdl,lmxa+1,pl(1,nsp),lmxa+1,qat(1,nsp))
...
C --- Optimise smooth-Hankel basis ---
        ...
        if (.not. cmdopt('--noopt',7,0,strn)) then
          ...
          if (z .gt. 0) then
cc     stop 'xxxxxxxxxxxxxxx end cccccxxx000'
c             write(stdo,*) '111 ',i,z,a,nr,rmax,nrmt,rmt,lmxa,pl,ql,nsp
c             write(stdo,*) '222 ',sum(v(1:nr)),sum(rofi(1:nr)),spid
...
c!! print out qbyl
c      open('qinrmt.'//trim(sname))
c      do  ib = 1, nbas
c        ispec=ssite(ib)%spec
c        write(ifqbyl,"(i3,10f12.6)") sspec(ispec)%lmxa, (sum(qbyl_rv(il,1:nsp,ib)),il=1,lmaxa)
c      enddo  
c      close(ifqbyl)
...
C ... Heap
      ...
      ipr = iprint()
c      if (z .lt. 0.99d0) lrel = 0 !takao think lrel here was not used may2021
...
C     ... Get center energy
     ...
     .      , e2 , qrmt )
c            print *,' ttt center e=',irep,eh,e2
C     ... Vary rsm
c            drsm = drsm0
c            call popta1 ( rsm + drsm , eh , l , z , rmt , nr , nrmt , rofi
c     .      , h_rv , v ( 1 , isp ) , a , b , enu , p , phi , dphi , phip
c     .      , dphip , e3 , qrmt )
c
c            call popta1 ( rsm - drsm , eh , l , z , rmt , nr , nrmt , rofi
c     .      , h_rv , v ( 1 , isp ) , a , b , enu , p , phi , dphi , phip
c     .      , dphip , e1 , qrmt )
c
c            call popta2(l,rsm,eh,drsm,e1,e2,e3,rlim1,rlim2,raddx,rnew,
c     .      stifr,jpr)
...
Cki#error ERROR, try to release name= opsi ,but list=null at linenumber= 1031 list= (None)
...
C  ... Get energy of this wave function
  ...
  261     format(' l    Rsm     Eh     Q(r>rmt)   Eval',
   ...
   10   continue
ctakao moved deallocation to here
        deallocate(gp_rv)
        deallocate(g_rv)
        deallocate(h_rv)
   80 continue
...
c      write(stdo,"(a)")'conf:-----------------------------------------------------'
c      sout = ' '
c      call awrit3('%?#(n>=30)#%N## coretail: q=%;3g, spill out=%;3g.',
c     .sout,len(sout),0,ipr,v0,q0)
...
c        call awrit2('%a  Fit with Hankel e=%;5g  coeff=%;5g',sout,
c     .  len(sout),-stdo,eh,c)
      endif
