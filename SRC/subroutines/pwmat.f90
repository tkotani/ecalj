...
!  endblock
  ...
  block
...
#endif
    call stopwatch_init(sw1, 'set ppovlx')
    call stopwatch_init(sw2, 'set_pwh')
    call stopwatch_init(sw3, 'get_pwhovl')

    call stopwatch_start(sw1)
    !ppovl_save is set in GPU in case of GPU version
    call set_ppovl(ngp, igv, ngmx, igvx, bas, rmax, nbas, alat, plat, qlat, ppovl_save)
    ...
    write(06,*) '**xxx ndimh, ngp, ngmx, napw', ndimh, ngp, ngmx, napw
...
subroutine set_ppovl(ng1, igv1, ng2, igv2, bas, rmax, nbas, alat, plat, qlat, ppovl_save)
  ...
  if(debug) write(06,*) 'get_ppovl_init: ', n1x, n1m, n2x, n2m, n3x, n3m
  !$acc kernels
  ppovl_save(:,:,:) = (0d0, 0d0)
  !$acc end kernels
