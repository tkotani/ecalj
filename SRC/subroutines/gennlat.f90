...
subroutine gennlat(pos,nbas,plat,nk1,nk2,nk3,npairmx,npair,nlat,nqwgt,nlatS,nlatE,ok)
  ...
  integer(8):: nlatS(0:nk1-1,0:nk2-1,0:nk3-1,nbas,nbas)
  integer(8):: nlatE(0:nk1-1,0:nk2-1,0:nk3-1,nbas,nbas)
  call tcn('gennlat')
  !      print *,'gennlat:'
  ok=.true.
  pi=4d0*datan(1d0)
        ...
        do ik1=0,nk1-1
           do ik2=0,nk2-1
              do ik3=0,nk3-1
                 ...
                 !          rmax2 = sum(pin*matmul(rxprod,pin)) + eps  ! eps is to make degeneracy safe.
                 !          nrmax(:) =  sqrt(rmax2*xmx2(:))+abs(pin(:)) ! range of ix used in shortn2
                 !          nmax =  nrmax
                 ! cccccccccccc
                 !          print *,'pin=',pin
                 !          print *,' rmax2xmx=',rmax2,nrmax,nmax
                 !          nmax=4
                 ! cccccccccccc
                 !          call shortn2(pin,nmax,rxprod,noutmx, nout,nlatout)
                 ! find smallest nlat for mod(posp+dx, {rxlat(:,1),rxlat(:,2),rxlat(:,3)})
                 !          write(6,"(3i4,' nmax=',3i4,' nout=',i3)")nd(1:3),nmax(1:3),nout
                 ...
                 !      T-->Tbar   Tbar=matmul(plat,nlat(:,nlatS:nlatE,ib1,ib2))
                 !                 phase =sum_{i=nlatS,nlatE} 1/(nlatE-nlatS+1) exp(-ik*Tbar)
                 ! ccccccccccc
                 do ix=1,nout
                    nlat(:,nnn+ix,ib1,ib2)= &
                         nd + [nk1*nlatout(1,ix), nk2*nlatout(2,ix), nk3*nlatout(3,ix)]
                    !                  qwgt(nnn+ix,ib1,ib2)= 1d0/nout
