...
contains
  subroutine m_setargs()
    use m_cmdopt_registry, only: load_cmdopt2_registry, load_cmdopt0_registry, validate_arglist
    integer:: iarg,iargc
    character(120) :: strn
    if(.not.init) return
    ...
    ! m_cmdopt_registry, then strict-validate the rest of the cmdline.
    ! arglist/narg are passed explicitly to keep m_cmdopt_registry from
    ! having to `use m_args` (which would create a circular USE chain).
    call load_cmdopt2_registry(narg, arglist)
    call load_cmdopt0_registry(narg, arglist)
    call validate_arglist(narg, arglist)
  ...
  subroutine m_setargsc(cname,prt) bind(C) !Pass narg and arglist from python instead of m_setargs
    ...
    ! Parse cmdopt2 / cmdopt0 entries + strict-validate (same as m_setargs).
    block
      use m_cmdopt_registry, only: load_cmdopt2_registry, load_cmdopt0_registry, validate_arglist
      call load_cmdopt2_registry(narg, arglist)
      call load_cmdopt0_registry(narg, arglist)
...
999 continue
  ...
  subroutine print_usage_and_quit(prgnam)
    ...
    write(6,'(a)') ''
    write(6,'(a)') 'Cmdline syntax (strict; unknown tokens abort at startup):'
    write(6,'(a)') '  <sname>                       positional, the ctrlg file extension'
    write(6,'(a)') '  --ctrlg:<dotted.path>=<value> TOML override (in memory; file untouched)'
    write(6,'(a)') '  --<flag>                      registered cmdopt0 (presence bool)'
    write(6,'(a)') '  --<flag>=<value>              registered cmdopt2 (=value, typed)'
    write(6,'(a)') '  Anything else aborts with "unknown option" (typo) or "retired syntax".'
    write(6,'(a)') '  The full c0_*/c2_* registry is in SRC/subroutines/m_cmdopt_registry.f90.'
    write(6,'(a)') ''
    write(6,'(a)') 'Inputs (lmf / lmfa / lmchk, only files actually read):'
    write(6,'(a)') '  ctrlg.<sname>.toml          main control file (TOML schema; GW sections incl. product basis at the end)'
    ...
    write(6,'(a)') ''
    write(6,'(a)') '    Retired (now abort): -v..., --[<path>]=..., --toml.<path>=<value>,'
    write(6,'(a)') '                         --pr=N, --time=..., --phispinsym,'
    write(6,'(a)') '                         -emin=, -emax=, -ndos=, -EfermiShifteV=.'
    write(6,'(a)') '                         Use --ctrlg:<path>=<value> or --<flag>=<value>.'
    write(6,'(a)') ''
