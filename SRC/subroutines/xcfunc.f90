...
contains
  ...
  ! This file includes kinds of the xc functionals in LDA and GGA.

  ! I think no other files contained such functionals in lm7K package.

  ! For given input density (and their gradients), this returns Exc and its gradients.
  ! The easypbe.F, K. Burke, May 14, 1996. is contained at the bottom of this file.

  ! Two kinds of evxcv is included. One is evxcv_new (lmf7.2), the other is evxcv_old (lmf7.1).
  ! They caused a little idfferences.


  ! for older version of smvxcm.F ==============================
  ...
  subroutine evxcv_old(rho,rhosp,n,nsp,lxcf,exc,excx,excc,vxc,vxcx,vxcc)
    ...
    ! ----------------------------------------------------------------------
!!!! This routine has a bug when rho>0 and rhosp=0  for XCFUN=2
    !i Inputs
    ...
    !o         := d (rho exc) /rho)
    !o   excx  :exchange-only part of exc
    !o   vxcx  :exchange-only part of vxc
    !o   excc  :correlation-only part of exc
    !o   vxcc  :correlation-only part of vxc
    !r Remarks
    !u Updates
    !u   25 Jul 08 Fixed bug in CA functional when rho>0 and rhosp=0
    !u    8 Feb 02 vxcx and vxcc (T. Miyake)
    !u   14 Jan 02 excx and excc (T. Miyake)
    ! ----------------------------------------------------------------------
    implicit none
    ! ... Passed parameters
    integer :: n,nsp,lxcf
...
200 continue
    if (nsp == 2) then
       do  210  i = 1, n
          if (rho(i) > rmin .AND. rho(i) >= rhosp(i)) then
             rs = (rho(i)*fpi3)**(-th)
             x = rs/aph
...
400 continue
  ...
  !C     implicit none
  !      integer lxcfun,ifi
  !      integer lxcf,lxcg,k,i1,i2
  !      character *80 sout

  !      lxcf = mod(mod(lxcfun,100),3)
  !      lxcg = lxcfun/100

  !      k = 0
  !      if (lxcf .lt. 1 .or. lxcf .gt. 2) k = -1
  !      if (lxcg .ne. 0) k = k - 10
  !      excsan = k
  !      if (ifi .le. 0) return

  !      if (k .lt. 0) call rxi('evxcv: unknown XC code:',lxcfun)
  !      if (lxcf .eq. 1) write(sout,101)
  !      if (lxcf .eq. 2) write(sout,102)
  !  101 format(' XC potential is Ceperly-Alder/Vosko')
  !  102 format(' XC potential is Barth-Hedin')
  !      call strip(sout,i1,i2)
  !      if (lxcg .eq. 0) write(sout(i2+1:),201) 'no'
  !      if (lxcg .gt. 0) write(sout(i2+1:),201) 'with'
  !  201 format(' (',a,' gradient corrections)')
  !      call strip(sout,i1,i2)
  !      write(ifi,'(a)') sout(1:i2)
...
50     enddo
  ...
  subroutine vxnlcc(n,nsp,rhop,rhom,grhop,grhom,ggrhop,ggrhom, &
       grho,grpgrm,grggr,grggrp,grggrm,vxc1,vxc2,exc)
    !- Same as vxnloc but exc also cut off to avoid blowup in exc
    implicit none
    ! Passed parameters
    integer :: nsp,n,i
    double precision :: rhop(1),rhom(1),grhop(1),grhom(1),grho(1), &
