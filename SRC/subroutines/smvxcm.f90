      subroutine smvxcm(ssite,sspec,nbas,lfrce,k1,k2,k3,smrho,!slat,
...
c      stdo = lgunit(1)
cChangenglob      nsp  = nglob('nsp')
c      nsp  = globalvariables%nsp
cChangenglob      lxcfun = nglob('lxcf')
c     lxcfun = globalvariables%lxcf
      call tcn('smvxc')
      lxcfun= lxcf
      ngabc = lat_nabc
      ng    = lat_ng
...
c      call defcc (osmrho,-k1*k2*k3*nsp)
      ...
      call smcorm ( nbas , ssite , sspec , ng , rv_a_ogv , cgh1_zv !slat , 
     . , cgh2_zv , lfoc1 , lfoc2 )
...
c        call dpadd(smrho_w,smrho,1,k123*nsp,1d0)
...
C --- Direct branch (lfoc2 .eq. 0) ---
      if (lfoc2 .eq. 0) then
        ...
        call smvxc2 ( 0 ,  nsp , lxcfun , vol , n1 , n2 , n3 , !slat ,
...
c        print *,'smvxcm:calling vxcnlm sum smrho=',sum(abs(smrho))
        call vxcnlm(lxcg,nsp,k1,k2,k3,smrho,!slat,
...
c$$$        call awrit8('%x smooth'//
c$$$     .  ' rhoeps =%;12,6D %?#n==2#(%;11,6D,%;11,6D)%N%7f#%2j#'//
c$$$     .  '  rhomu =%;12,6D %?#n==2#(%;11,6D,%;11,6D)#%2j#',outs,120,
c$$$     .  0,rhoeps(1)+rhoeps(2),nsp,rhoeps,rhoeps(2),rhomu(1)+rhomu(2),
c$$$     .  nsp,rhomu,rhomu(2))
c$$$        call awrit5('%a%?#n==2#%N%7f#  #'//
c$$$     .  'avg vxc =%;12,6D %?#n==2#(%;11,6D,%;11,6D)',outs,len(outs),
c$$$  .  -stdo,nsp,(vxcavg(1)+vxcavg(nsp))/2,nsp,vxcavg,vxcavg(2))
        do i=1,nsp
