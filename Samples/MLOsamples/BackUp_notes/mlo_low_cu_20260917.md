...
新旧の違いの模式図（SmP 風）。青 = 射影子から外す帯。旧 (a) は価電子帯の底の 1 本が
「模型重み < 1/2 になる k だけ」外され、他の k では残る — 射影子の中身が k で変わる。
新 (b) はその帯を全 k で残し、常に半芯 + P 3s だけを外す。

![old per-k vs new min-over-k, schematic](mlo_nskip_schematic_20260918.png)
