...
**23:00 `--skipq0Sc` 試験**（`6ad2d1240`; Γ セル kx=1 の W を Σc から外す診断。6³ LDA からの一発、
χ₀+Σ 1000 K、kt1 `runs/oneshot1_666_skipq0_20260918/{REF,SKIPQ0}`）:
...
（副産物: hgw の W-build が 2 本同居時に `__c_mcopy8` で segfault、単独では通る。潜在バグ、要追跡。）

**次**: (1) ⟨48|Σc(ε₄₈)|33⟩ を q（W の kx）ごと・実軸/虚軸ごとに分解する（hsfp0 の
