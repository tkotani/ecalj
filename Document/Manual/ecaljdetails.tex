...
\label{sec:theory}
The {\tt ecalj} is based on the PMT method (=Linearized APW+MTO method)
\cite{kotani2015pmt,kotani_linearized_2013,pmt1}, which is
a mixed basis method. As long as we know, it is only a method using two
kinds of augmented waves simultaneously.
With {\tt ecalj}, we can do total energy calculation and 
atomic-position relaxations within LDA, GGA and LDA/GGA+U.
We can add spin-orbit coupling and so on (some limitations).

Especially, its uniqueness is in the QSGW calculations (quasiparticle
self-consistent GW). Since it is on top of the PMT method, we call the
QSGW method in ecalj as the PMT-QSGW method \cite{kotani_quasiparticle_2014}. 
In addition, we can calculate linear responses (dielectric and magetic), 
Wannier functions, and so on.

Originally the QSGW had implemented in the LMTO
\cite{kotani_quasiparticle_2007}, say, the LMTO-QSGW. 
However, the LMTO-QSGW is very difficult to use.
In contrast, PMT-QSGW is rather easier to use.
...
We use unit \alat\ (measured by a.u.=0.528177\AA) to represent length in the code.
Thus, to convert quantities in the unit of a.u., we
multiply \alat\ to the quantities of scales.
...
\end{verbatim}
 ...
 \item Use module. In the PMT part (one-body part)) \verb#ecalj/lm7K/#, we use strucrue. But no
       structures in the GW part \verb#ecalj/fpgw# part. I recommned to
       use modules.

 \item Module names are \verb#m_foobar#. Use 'only' option when you use
       a module.
 \item 
\begin{verbatim}
An exmple is 
...
\end{verbatim}
 ...
 \item Be careful about how to clarify the i/o of modules.
...
\section{Module programming for developers}

Dividing a long complex code into a main program and separate moldules is convenient to manage the code. By doing so, 
\begin{itemize}
...
\begin{itemize}
 ...
 \item Use {\it labels} for loops to avoid confusion. : 
 \begin{verbatim}
 ...
      do 
...
\begin{enumerate}
 ...
 \item 
     $\langle E^\bfk_\mu|\bar{\Pi}^\bfk |E^\bfk_\nu \rangle$ for Q0P is
       treated as the expantion near $\bfk=0$.

   (Need further explanation...)
 \item 
     Calculate $\frac{\partial^2 v^\bfk(R_\alpha-R'_\beta)}{\partial {R_\alpha} \partial {R'\beta}}$,
 \item 
 ...
 \item
   ...
   \begin{eqnarray}
   \bar{C}^{\rm N} = \frac{\partial^2 W^\bfk(R_\alpha-R'_\beta)}{\partial {R_\alpha} \partial_{R'\beta}}
   \end{eqnarray} on regular mesh points.
   And non-analytic part $\bar{C}^{\rm NA}$.
  \item
...
we have to use ``Screened Model Hamiltonian'' $\HSM$ 
instead of \req{eq:hm}. 
%This is because TDFT do not
%include the screening effects when we evaluates spin susceptibility;
When we calculate the ladder diagram for spin susceptibility,
we should use the screened interaction by the polarization. 
In fact, usual first-principles calculations use $\WM$ for 
the calculation of spin susceptibilities \cite{di_valentin_spin_2014}.
The screened model Hamiltonian is given as \begin{eqnarray}
\HSM= \HMZ + \USM - \SHFSM, 
\label{eq:hsm}
\end{eqnarray}
where we use the screened Coulomb interaction $\USM$ 
instead of $\UM$ in \req{eq:hm}. 
If we use the usual RPA, we have $\USM=\WD$.
This is different from \req{eq:hm}.
%\begin{eqnarray}
%\USM = \frac{1}{1-U_{\rm M} \PM} U_{\rm M},
%\label{eq:USM}
%\end{eqnarray}
%that is, we just use $\USM=\WM$.
We re-calculate the ground state by HF within the model, 
and the linear response for the system in the TDHF.
Thus the ground state and the linear response are consistent.\\

Wannier modeling, 
Tetrahedron method, 
Hilbert transformation.

%However, $\USM=\WM$ includes the self-interaction effect which should
%be removed. It is not so straightforward to remove the self-interaciton effect.
%What we can do is evaluating 
%the proper polarization $\PM$ as the sum of the ladder diagram,
%%%%%where we use the interaction $W$.
...
\section{Downfolding}
Downfolding is a general concept which often appears in physics of varieties of contexts. It is based on the Block inversion Eq.\ref{eq:blockinv}.
