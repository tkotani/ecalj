...
subroutine rsibl(ssite,sspec,lfrce,nbas,isp,q,iq,ndimh,nspc,& 
  ...
  call gvlst2(alat,plat,q,n1,n2,n3,0d0,gmax,0,000,0,ng,xx,xx,xx,xx)
...
subroutine rsibl4(vol,ng,nspc,gq,vpsi,psi,f0)
  ...
  complex(8):: vpsi(nspc,ng),psi(nspc,ng)
  integer :: ispc,ig
  real(8):: sum1,sum2,sum3,xx(nspc,ng)
  xx = dimag(vpsi(:,:))*dreal(psi(:,:)) - dreal(vpsi(:,:))*dimag(psi(:,:))
  f0(:) = 2*vol*sum(matmul(xx(:,:),gq(:,:)),dim=1)
