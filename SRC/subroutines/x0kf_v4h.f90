...
module m_x0kf
  ...
  logical:: debug = .false.
contains
  function X0kf_v4hz_init(job,q,isp_k,isp_kq, iq, crpa, ikbz_in, fkbz_in) result(ierr) !index accumulation
    implicit none
    ...
    AccumulateIndex4icount: do 110 k = 1,nqbz
     ...
     flush(stdo)     !     if(ipr) write(stdo,*)'mm111mmmmmmm22222aaa',nqbz,k,nkqmin(k),nkqmax(k),'job=',job,nbnb(k,1)
...
!        if(GPUTEST) then
          ...
          kloop:do 1500 k=k_lo,k_hi !zmel = < M(igb q) phi( rk it occ)|  phi(q+rk itp unocc)>
            ! Omega+k parallelism: iw restricted to iw_lo:iw_hi; k restricted to k_lo:k_hi.
            if(tetwtk) then
      ...
      HilbertTransformation:if(isp_k==nsp .OR. chipm) then
        ...
        mpi_k_accumulate: block
          ...
          if (mpi__size_k == 1) then
            ...
            enddo
            ! root_k copies its flat omega slice into shm_wvr (host, shared memory).
            if (mpi__root_k) then
              do iw = iw_lo, iw_hi
                if (iw == 0) cycle
