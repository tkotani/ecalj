...
!! (only epsPP_magnon_chipm mode works).
     ...
     &     readscr, checkorb, !, getqsym,
     &     checkorb2, diagwan, diagwan_tr, wan_imat,
...
! Feb2020 QforEPS mode
      ...
      integer:: idammy
c      integer, allocatable:: epslgroup(:)
...
c$$$      open (newunit=ifq0p,file='Q0P')
c$$$      read (ifq0p,"(i5)") nq0i
c$$$      write(6,*) ' ### nqibz nq0i=', nqibz,nq0i
c$$$      allocate( wqt(1:nq0i),q0i(1:3,1:nq0i),epslgroup(nq0i) )
c$$$      do i=1,nq0i
c$$$        read (ifq0p, * ) wqt(i),q0i(1:3,i),idammy,epslgroup(i)
c$$$      enddo
c$$$      close(ifq0p)
c$$$      nq0ix = nq0i
c$$$      do i=1,nq0i
...
!! optional npmtwo is added aug2017
      !! 20190604 Im[K]
c$$$      if (negative_cut) then
c$$$         call getfreq(epsmode,realomega,imagomega,tetra,omg2max,wemax,niw,ua,MPI__root, npmtwo=.false.)
c$$$      else
c$$$         call getfreq(epsmode,realomega,imagomega,tetra,omg2max,wemax,niw,ua,MPI__root, npmtwo=.true.)
c$$$      endif
      niw=niw_in
...
!! Wannier eigenvalues are read
      do is=1,nspinmx
         !call wan_readeigen(qbz(:,:),nqbz,is) !QforEPSL ver. (20Feb, 2020)
...
!!! write qshort instead of q
         !wibz: BZ weight 
c         wibz=0d0 !! not valid this version 20Feb,2020
...
ccccccccccccccc simple band for check
     ...
     o           ev_w2(1:nwf,kx), evc_w2(1:nwf,1:nwf,kx))            
            ...
            if (cma_mode) then
c$$$               if (iq==1) write(6,*) "cma_mode: True"
c$$$               if (iq==1) write(6,"('cma_mode: iwf_s, iwf_e',2i4)") cma_iwf_s,cma_iwf_e
c$$$               if (iq==1) write(6,"('cma_mode: cma_up_shift, cma_dn_shift',2E13.5,' [eV]')") cma_up_shift,cma_dn_shift
               do iwf = 1, nwf
...
c$$$     i        qbas,ginv, ef, nqibz, nwf,ev_w1,ev_w2, nctot,wan_ecore,
c$$$     i        nqbz,qbz,nqbzw,qbzw,  ntetf,idtetf,ib1bz,
c$$$     i        nwf,ebmx,mtet,eibzmode,wan) !nov2016
...
c$$$            do iwf=1,nnwf
c$$$               do jwf=1,nnwf
c$$$                  zxq(iwf,jwf,iw)=cmplx(dble(zxq(iwf,jwf,iw)),0d0,kind(0d0))
c$$$               enddo
c$$$            enddo
c$$$         enddo
c$$$      endif
...
!     &        iw,www*2d0,trmat/hartree
     &           iw,www*2d0,hartree*sum(eval_wk)/(znorm)         
            ...
            if (output_ddmat) then
               ...
               !call writeddmat(zxq(:,:,:),nwf,nw_i,nw,"wan_ChiPMr.mat.do",.false.)
...
cc W shift (q is far from Gamma)
cc W shift for Cu2MnAl
         if (cma_mode) then
            scrw = scrw_original
...
ccc Hermite check for 1-eWK for developing code
         if (write_hmat) then
            if (iq==4 .and. iw==205) then
...
c     &           www*2d0,sum(eval_wk(1:nnwf))/hartree
...
cccc Hermite check (zxq and rmat) for developing code
         if (write_hmat) then
            if (iq==4 .and. iw==205) then
