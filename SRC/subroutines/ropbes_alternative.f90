! ----------------------------------------------------------------
!   Bessel function at r(i)*sqrt(e) divided by r**l
...
!     CODED BY T.OGUCHI  10/01/87

!     LMAX : MAXIMUN ANGULAR MOMENTUM
!     X    : ARGUMENT OF BESSEL FUNCTION
!     F    : SPHERICAL BESSEL FUNCTION
!     D    : DERIVATIVE OF SPHERICAL BESSEL FUNCTION

subroutine bessel(lmax,x,f,d)
  !                           @(#)bessel.f 9.1 97/05/08 14:47:47
  IMPLICIT REAL*8 (A-H,O-Z)
  DIMENSION F(0:LMAX),D(0:LMAX)
  DIMENSION Q(0:50)
  IF (LMAX > 50) STOP '=== STOP AT BESSEL ==='
  IF(X < 1.0D-3) THEN
     XN=1.0
     F1=1.0
     F2=3.0D0
