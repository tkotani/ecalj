...
c      if(debug) write(6,"('  alpm=',3f12.6)") alpm
      if(iq0pin==1) then
...
c          do i=1,nq00i
c            write(*,'(" initial q0x=",i3,3f9.3)')i,q0x(:,i)
c          enddo
...
!!
     ...
     &       iq0i,sum(rrr*matmul(emat,rrr))
          ...
          enddo
          deallocate(cy,yl)
c          lxklm=6         !this is used for inversion procedure in hx0fp0.sc.m.f
c          nnnt=n1q*n2q*n3q
