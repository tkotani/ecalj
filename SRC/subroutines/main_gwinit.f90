!> Generate the GW-related part of ctrlg.<sname>.toml.
...
!                          replaced with freshly-generated ones; [product_basis]
!                          closes the file and carries the per-atom tables
!                          nlx/valence/core that used to be PB.<sname>.toml)
!
!  Atomic write: the output is first written to <name>.partial, then
...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    character(len=:), allocatable :: ctrlg, ctrlg_partial
    character(len=*), parameter :: rule = '# '//repeat('-', 64)
    ...
    write(ifi,'(a)') '# ----- Finite electronic temperature (off unless you uncomment; see manual/kBT) -----'
    write(ifi,'(a)') '# t_tetrakbt = 1000   # (K) chi0 with Fermi-Dirac occupations at this T (finite-T tetrahedron, method'
    write(ifi,'(a)') '#                     # B-prime) and the finite-T Fermi level EFERMI_kbt; 0 (default) = T=0 tetrahedron'
    write(ifi,'(a)') '# t_sigmakbt = 1000   # (K) Fermi-Dirac occupations in Sigma too; set equal to t_tetrakbt (needs t_tetrakbt>0)'
    write(ifi,'(a)') '# wcsmear    = true   # smear the pole position of the real-axis term of Sigma_c with the same kernel as'
    write(ifi,'(a)') '#                     # the occupation (esmr / kBT); needed for metals with sharp plasmons (manual/kBT)'
    ...
    write(ifi,'(a)') '# the model per atom; the three keys shape the weight'
    write(ifi,'(a)') '#   theta = sigma((eps - ecut_j)/mlo_w),  ecut_j = max(CBM + mlo_delta, eps^MTO_j)'
    write(ifi,'(a)') '# mlo_delta: how far above the band edge (EF in metals) the model must be'
    write(ifi,'(a)') '#   accurate. A statement of what you want, not a fitting parameter.'
    write(ifi,'(a)') '# mlo_w: width of the fall-off above that floor. THIS is the knob to turn if'
    write(ifi,'(a)') '#   the residual is too large. Measured optima: semiconductors ~2, Fe/Cu ~11.'
    ...
    write(ifi,'(a)')
    !! ----- per-atom tables (nlx / valence / core), formerly PB.<sname>.toml -----
    write(ifi,'(a)') rule
    write(ifi,'(a)') '# nlx [iatom, l, nnvv, nnc]: one row per (iatom, l). iatom = site index in'
    write(ifi,'(a)') '#   [[site]] order (lmchk prints it). nnvv = radial functions for valence at'
    write(ifi,'(a)') '#   this l (2 = phi, phidot; 3 = phi, phidot, phiz local orbital); nnc = number'
    write(ifi,'(a)') '#   of core levels at this l. Derived from rsmh/eh/pz of [[spec]].'
    write(ifi,'(a)') 'nlx = ['
    do ibas = 1, nbas
       do lk = 0, lmxaa(ibas)
    ...
    !! valence rows
    write(ifi,'(a)') rule
    write(ifi,'(a)') '# valence [iatom, l, n, occ, unocc]: one row per valence radial function.'
    write(ifi,'(a)') '#   n = 1 phi, 2 phidot, 3 phiz (local orbital). occ / unocc = 1/0 put the'
    write(ifi,'(a)') '#   function into the occupied / unoccupied group; the product basis is built'
    write(ifi,'(a)') '#   from (occupied) x (unoccupied) products and pruned by pb_tolerance.'
    write(ifi,'(a)') '#   phidot is usually left out (0 0) for speed; including it is more accurate.'
    write(ifi,'(a)') 'valence = ['
    iatbk = 0
    do ibas = 1, nbas
       do lk = 0, lmxaa(ibas)
    ...
    !! core rows
    write(ifi,'(a)') rule
    write(ifi,'(a)') '# core [iatom, l, n, occ, unocc, forX0, forSxc]: one row per core level.'
    write(ifi,'(a)') '#   n = core index at this l (1 = deepest). occ / unocc as above; forX0 / forSxc'
    write(ifi,'(a)') '#   = 1/0 include this core in chi0 / in Sigma_xc (CORE1/CORE2 of PRB 76, 165106).'
    write(ifi,'(a)') '#   Modern runs leave every core row all-zero.'
    write(ifi,'(a)') 'core = ['
    do ibas = 1, nbas
       do lk = 0, lmxaa(ibas)
          if (lk == 0) seg2 = 'S'
    ...
    !! A PB.<sname>.toml from before 2026-09 is no longer read when ctrlg carries
    !! the tables; say so rather than silently leaving two copies around.
    inquire(file='PB.'//trim(sname)//'.toml', exist=stale_pb)
    if (stale_pb) write(6,'(a)') ' gwinit: NOTE PB.'//trim(sname)//'.toml is obsolete;'// &
         ' its tables are now in '//ctrlg//' [product_basis]. Remove the old file.'

    stop ' OK! gwinit upserted [gw]/[mlo]/[blocks]/[product_basis] (with nlx/valence/core) into '//ctrlg
