...
contains
  ...
  subroutine lmaux()        !main part of lmchk
    ...
    integer :: getdig,i,ip,j,k,m,ifi,iprint,lpbc, &
         ...
         nbaspp,nkd,nkq,nspec,neul,nc,mxcsiz,nttab,igets, &
    ...
    double precision :: xv(10),xx,alat,plat(3,3),facrmx,facrng,&
...
345       format(i4,2x,a8,f10.6,2f11.6,1x,3f11.6)
  ...
  subroutine makrm0(opt,nspec,nbas,alat,plat,pos,slabl,ips,modep, &
    ...
    ! ----------------------------------------------------------------------
    !i Inputs
    !i   opt  :specifies options in makrm0.
    !i         :
    !i         :1s digit : prescription for constructing potential
    !i         :  The only option is mode 1 (see Remarks):
    !i         :  set 1's digit opt=1.
    !i         :
    !i         :10s digit : quantity to overlap
    !i         :  0 overlap electrostatic atomic potentials
    !i         :  1 overlap estat atomic potentials + add total xc potential
    !i         :    This option tends to produce slightly more uniform radii.
    !i         :  2 overlap total atomic potentials
    !i         :    This option probably doesn't make sense.
    !i         :
    !i         :100s digit specifies what MT radii are returned.
    !i         :Initially MT radii are generated by site.
    !i         :  0 return MT radii by site:
    !i              rmt(i) corresponds to radius for site i
    !i         :  1 average MT radii by species:
    !i              rmt(i) corresponds to radius for species i
    !i         :  2 smallest MT radii of this species
    !i              rmt(i) corresponds to radius for species i
    !i         :
    !i         :1000s digit specifies whether nspec,slabl,ips refer to
    !i                species or classes (used for printout only)
    !i         :  0 quantities refer to species
    !i         :  1 quantities refer to classes
    !i   nspec :number of species (or classes)
    ...
    !i   slabl :vector of species (or class) labels
    !i   ips   :species (or class) table: site ib belongs to species ips(ib)
    !i   modep :integer vector of length 3 governing how lattice vectors
    !i         :are shortened (shorps).  In particular,
    !i         :0 suppresses shifts along plat(j)
    !i         :2 shifts to minimize length of pos
    !i   z     :table of nuclear charges by species (or class)
    !o Outputs
    !o   rmt
    !l Local variables
    !l         :
    !r Remarks
    !r   makrm0 attempts to determine an optimal set of muffin-tin radii so
    !r   that the errors due to the muffin-tin approxiation are minimized.
    !r   Inside the MT-spheres the potential should be as spherical as
    !r   posssible, and the potential should be as constant as possible.
    !r
    !r   At present there is only one prescription (mode 1).
    !r   This routine follows the methodology of the Stuttgart code potmax.
    !r
    !r   mode 1 : rmt are chosen as follows:
    !r   1. The free-atom potential is constructed for all species.
    !r   For each atom:
    !r   2. A neighbor table is made for the atom.
    !r   3. The connecting vectors to each neighbor make up a group
    !r         of direction vectors stellating from that atom.
    !r   4. The first occurence of a maximum in the potential along any
    !r         of these vectors determines rmt for the atom
    !r
    !r   Finally, rmt may be averaged over species (or classes).
    !u Updates
    ...
    ! --- Densities and potentials for each species ---
    exi(1) = -1
    exi(2) = -2
    exi(3) = -4
    exi(4) = -6
    ...
    do  is = 1, nspec
       ...
       !       nrspec(is) = iabs(iclbsj(is,ips,-nbas,nbas))
       spid = slabl(is)
       rsmfa = 1
       rfoca = 1
       qcor(1) = 0
       ...
       !       Restore 4*pi*r**2*(total density) in array rho
       call daxpy(nrmx*nsp,1d0,rhoc(1,is),1,rho(1,is),1)

       !       Use estat potential: overwrite v with estat
       if (opt1 <= 1) then
          !         call prrmsh('vtot',rofi,v(1,is),nr,nr,1)
          vrmax = 0
          call poiss0(z(is),a(is),b(is),rofi,rho(1,is),nr,vrmax,v(1,is), &
               xx(2),xx(4),nsp)
          !         call prrmsh('ves',rofi,v(1,is),nr,nr,1)
       endif
       ...
       !       Overwrite v,rho with proper spherical potential and density
       do  ir = 2, nr
          v(ir,is) = v(ir,is) - 2*z(is)/rofi(ir)
          rho(ir,is) = rho(ir,is)/(fpi*rofi(ir)**2)
       enddo
       !       call prrmsh('rho',rofi,rho(1,is),nr,nr,1)
       !       call prrmsh('v',rofi,v(1,is),nr,nr,1)
    enddo


    ! ... Sets range for neighbor table
    do  ib = 1, nbas
       is = ips(ib)
       range(ib) = facr * rmtl(is)
    enddo

    ! --- For each site, find rmti = initial estimate for rmtl ---
    do  ib = 1, nbas

       is = ips(ib)
       spid = slabl(is)

       !   ... Neighbor table connecting sites to this one
       nttab = mxcsiz
       ...
       !   ... Maximum in potential ( mode 1)
       !       Find first maximum along all direction vectors
       !       which are proportional to rpos, excluding self (first site).
       !       As a conservative choice, choose initial radius = rmtl/4.
       np = nttab-1
       if (nttab > npmx) &
       ...
       !       Re-entry for loop until maximum in potential found
       pass = 0
30     continue
       !       Make nttab-1 connecting vectors of radius ri
       do  k = 2, nttab
          fac = ri/dsqrt(ddot(3,rpos(1,k),1,rpos(1,k),1))
          call dpcopy(rpos(1,k),xp(1,k-1),1,3,fac)
       enddo

       !       Add superposition of potentials for each vector
       call sumsro(xp,np,ips,a,b,v,nttab,iax,rpos,vp(1,2))

       !       Add vxc[superposition of densities] for each vector
       if (opt1 == 1) then
       ...
       !       Check against prior point.  Skip if nothing yet to compare
       if (pass > 0) then
          do  k = 1, nttab-1
             !           This loop executes if a maximum is found
             if (vp(k,1) > vp(k,2)) then
                !             It shouldn't happen already at the first point
                if (pass == 1) then
                   call &
                ...
                if (pass > 1) then
                   ...
                   di = -slo/cur
                   if (dabs(di) < 1) then
                      !                 print *, pass, di, ri, (ri/facri)*dexp(di*(facri-1))
       ...
       !       Copy vp(*,2) to vp(*,1), and increment pass
       ...
       !       Loop exit: radius rmti(ib) has been found
31     continue

       !       This is the connecting vectory along which the maximum was found
       !       iconn = iax(2,k+1)

    enddo

    ! --- Copy appropriate rmti to final result, depending on opt2 ---
    strn = 'estat'
    if (opt1 == 1) strn = 'LDA'
    ...
    ! ... First loop for printout only, so can hang on to original rmt
    do  ib = 1, nbas
       is = ips(ib)
...
10  enddo
  ...
  subroutine ppair1(isw,ib1,ib2,nbasp,ips,alat,plat,pos,range, &
       nttab,ntab,iax,mxcsiz)
    !- Fill out parts of the aix table
    ! ----------------------------------------------------------------
    !i  Inputs
    !i   isw   :1's digit fixes how range is calculated.
    !i           0: vector length must be < range(i)+range(j)
    !i           1: include all connecting vecs w/ r < range(i)
    !i         :10's digit sets what part of iax table is calculated
    !i           0: make iax(6),iax(7),iax(10)
    !i           1: make iax(7),iax(10)
    !i           2: no change to iax: printout only
    !i           4: just make iax(6)
    !i   ib1   :fill out iax table for pairs ntab(ib1)+1..ntab(ib2)
    !i   ib2   :fill out iax table for pairs ntab(ib1)+1..ntab(ib2)
    !i   nbasp :size of padded basis (not needed)
    !i   ips   :species table: site ib belongs to species ips(ib)
    !i   alat  :length scale of lattice and basis vectors, a.u.
    !i   plat  :primitive lattice vectors, in units of alat
    !i   pos   :basis vectors
    ...
    !i          This quantity may be a scalar, a species-dependent
    !i          array, or a site-dependent array, depending on ips(1);
    !i          see description of ips.  See 1s digit of isw for
    !i          how range is used.
    !i   nttab :total number of pairs in neighbor and iax (pairc.f)
    !i   ntab  :ntab(ib)=offset to neighbor table for cluster ib (pairc.f)
    !i   iax   :neighbor table containing pair information (pairc.f)
    !i   mxcsiz:maximum cluster size (for printout only)
    !o  Outputs
    !o   iax(6)   :index to conjugate (jb,ib) pair matching (ib,jb)
    !o             NB: only matching pairs within site list can be found.
    !o   iax(7)   :permutation index ordering cluster by increasing
    !o             effective site index; see ppair4.f
    !o   iax(10)  :effective site index; see siteid.f
    ! ----------------------------------------------------------------
    ...
    if (mod(isw1,2) == 0) then
       do 170  ib = ib1, ib2
          do  70  it = ntab(ib)+1, ntab(ib+1)
             if (iax(6,it) /= 0) goto 70
             jb = iax(2,it)
             !   ... No matching pair for padded sites
             if (jb < ib1 .OR. jb > ib2) goto 70
             do  72  jt = ntab(jb)+1, ntab(jb+1)
                !         if (iax(2,jt) .eq. ib) then
                !         endif
                if (iax(2,jt) == ib .AND. &
...
170    enddo
    ...
    ! --- Printout ---
80  if (ipr < 30) goto 91
    if (ipr <= 40) write(stdo,'(1x)')
    if (ipr > 40) write(stdo,332)
...
10  enddo
  ...
  subroutine ovmin(sovmin,nbas,nbasp,alat,plat,rmax,rmt,dclabl, &
    ...
    integer :: i1mach,isw,ir,i,j,j1,j2,ls,m,lstyle, &
