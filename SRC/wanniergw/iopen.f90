...
c iopen = file number, starting from unit 10
      ...
      logical :: debug=.false.
c$$$c check that file is not already opened oct2005
c$$$      if(debug) then
c$$$        write(6,*) '------iopenstart ---------'
c$$$        do   i = 1,namnmax
c$$$          write (6,"(i3,' ',a)") i+9,fnamn(i)
c$$$        enddo
c$$$      endif
...
c        write(6,*)' namnxxx=',namn,namnx,fnamn(namn)
...
cccccccccccc
c      write(6,*)'namn,namnmax=',namn,namnmax
ccccccccccccccc
ctakao
      if(debug) then
        do ix=9+1,namnmax
