...
c        mtett=.true.
c        if(sum(abs(mtet))<3) 
c     &  print *, ' we use devided-tetrahedron scheme mtet=',mtet
c      else
c        mtett=.false.
c      endif
c$$$      if(mtett) then
c$$$        ifmtet=ifile_handle()
...
c$$$          call rx( 'tetwt5: nband_x /=nband')
c$$$        endif
c$$$        allocate( eigtet(nband,nqnumm,nsp) )
c$$$        do iqx=1,nqnumm
c$$$          do ispx=1,nsp
c$$$            read (ifeig) eigtet(1:nband,iqx,ispx)
c$$$            if(verbose()>150) write(6,"('iq=',i3,'  eig(1:5)=',5f10.4)")iqx,eigtet(1:5,iqx,ispx)
c$$$          enddo
c$$$        enddo
c$$$        close(ifeig)
c$$$      else
...
c$$$            ekzz2(1:nband, kx) = eigtet(1:nband,kqxxm,isp2) !ekzz1(1:nband, kqxxm)
c$$$          enddo
c$$$        else
c$$$          do kx = 1, nqbzm
...
c$$$          enddo
c$$$        endif
c$$$        deallocate( eigtet)!,index_qbzm)
c$$$      else
        ekzz1=ekxx1
        ekzz2=ekxx2
c$$$      endif
...
c     call getkeyvalue("GWinput","defermia",dmua,default=0d0)
c     call getkeyvalue("GWinput","defermib",dmub,default=0d0)
c     call getkeyvalue("GWinput","exband",exb,default=.false.)
c     efermia = efermi + dmua
c     efermib = efermi + dmub
c      write(6,*) "defermia, defermib:", dmua,dmub
c      write(6,*) " efermia,  efermib:", efermia,efermib
      efermia = efermi 
      efermib = efermi
...
c          noccx_k = noccx1 ( ek_(1:nband, 0:3),4,nband, efermi)!the highest number of occupied states
c          noccx_kq= noccx1 (ekq_(1:nband, 0:3),4,nband, efermi)
...
!if( maxval(eunocc(:)-eocc(:)) <dmub-dmua ) cycle ! this makes a bit effective.
...
c------------------------------------------------------------------------
      subroutine lindtet6(
     i  kkv,kvec, ea, eb, x, efermia, efermib,      ! kvec(1:3, 0:3), ea(0:3), x(0:3) 
     i  frhis, nwhis,  
     o  wtthis ) 
c     m  fermi_ds)
...
!! ds is the accumulation variable for the fermi surface.
     ...
     &  vcell,efermi,etest ,  kkv(3,4), efermia, efermib
      integer(4):: nwhis
...
c#else
c      call sortea( ea,ieaord, 4 ,isig); ieaord(1:4) = ieaord(4:1:-1)
c#endif
...
!!  Rath&Freeman Integration of imaginary part on Eq.(17)
     ...
     & V1,V2,V3,V4,D1,D2,D3,D4,e1,e2,e3,e4
      integer(4)::i1,i2,i3
c-------------------------------------------------------------
c      if((.not.(e(1)<=e(2))).or.(.not.(e(2)<=e(3))).or.(.not.(e(3)<=e(4))) ) then
c        write(6,"(/,' --- integtetn wrong order of e=',4d14.6)") e
c        call rx('integtetn: wrong order of e')
c      endif
