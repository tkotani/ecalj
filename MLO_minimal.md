...
`mlo_method = 4` の既定値($\Delta=0.18$ Ry、$w=0.20$ Ry)での実測。
...
|---|---:|---:|---:|---:|---:|
| C (sp のみ, 8) | 0.0149 | 0.0039 | 0.007 | −18 | 0.78 |
| Cu (spd, 9) | 0.0295 | 0.0065 | 0.085 | — | — |
| Fe (spd, 9) | 0.0171 | 0.0111 | 0.039 | — | — |
| SrTiO₃ (15) | 0.0211 | **0.0529** | 0.018 | +28 | 1.14 |
| NiO (17) | 0.0216 | 0.0319 | 0.039 | +14 | 1.07 |
| Si $6^3$ (19) | 0.0198 | 0.0076 | 0.015 | +25 | 1.18 |
| Si $8^3$ (19) | 0.0151 | 0.0028 | 0.009 | **+5** | **1.00** |
| C (spd, 19) | 0.0137 | 0.0034 | 0.003 | −1 | 1.04 |
| GaAs (19) | 0.0174 | 0.0026 | 0.012 | +29 | 1.02 |
| FeCo (20) | 0.0062 | 0.0035 | 0.014 | — | — |
| RuO₂ (26) | 0.0095 | 0.0095 | 0.035 | — | — |
| Al₂O₃:Cr (53) | 0.0175 | 0.0030 | 0.007 | +7 | 1.08 |
| **平均**(Si は $6^3$) | **0.0171** | **0.0124** | **0.025** | | |

ΔE rms は eV。金属に CBM は無いのでギャップと $m^*$ は「—」。
$m^*$ 比 $=m^*_{\rm MLO}/m^*_{\rm DFT}$ で **1.00 が一致**(誤差ではなく比)。
正規化した損失ベクトルは §3 にある。

目につくのは 3 点。**SrTiO₃ の占有側 0.0529 eV** が突出しており(窓内は 0.0211 と普通)、
窓だけ見ていたら見落とす種類の誤差である。**Si は $6^3$ と $8^3$ で別物**
(ギャップ +25 → +5 meV、$m^*$ 1.18 → 1.00)で、$6^3$ の値は補間の雑音を含む。
**C.sp の $m^*$ 比 0.78** は sp のみ 8 軌道というミニマル模型の限界(§4)。
