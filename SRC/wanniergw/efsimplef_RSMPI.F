...
c
c------------------------------------------------------------
c May 2007 Rei Sakuma : modified for MPI version
c    only process with 
c        Is_IO_Root_RSMPI()==.true.(see gwsrc/RSMPI_mod.F)
c      prints  messages and open output files.
c------------------------------------------------------------
...
c total valence charge
      if(legas) then
c RS: legas mode is not implemented..
...
c RS: only ioroot opens these files
      if (Is_IO_Root_RSMPI()) then
        if8301=ifile_handle()
        if8302=ifile_handle()
...
c        if(wwg<valn+2d0) write(6,*) ik,ieaord(ik),ektx(ieaord(ik)),wwg
