...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    write(ifpb,'(a)') '# PB.<sname>.toml -- auto-generated by gwinit'
    write(ifpb,'(a)') '# Per-atom product-basis tables. Loaded by m_GWinput with ctrlg.<sname>.toml'
    write(ifpb,'(a)') '# (pb_tolerance and pb_lcutmx live there; do not duplicate them here).'
    write(ifpb,'(a)') '# iatom = site index in [[site]] order (lmchk prints it); l = angular momentum.'
    write(ifpb,'(a)') '# Same meaning as the legacy <PRODUCT_BASIS> block; only the container changed.'
    write(ifpb,'(a)') '#'
    write(ifpb,'(a)') '# nlx     [iatom, l, nnvv, nnc]          derived from rsmh/eh/pz of [[spec]]'
    write(ifpb,'(a)') '#           nnvv : radial functions for valence at this l: 2 = {phi, phidot},'
    write(ifpb,'(a)') '#                  3 = {phi, phidot, phiz (local orbital)}'
    write(ifpb,'(a)') '#           nnc  : number of core levels at this l'
    write(ifpb,'(a)') '#'
    write(ifpb,'(a)') '# valence [iatom, l, n, occ, unocc]      one row per valence radial function'
    write(ifpb,'(a)') '#           n     : 1 = phi, 2 = phidot, 3 = phiz (local orbital)'
    write(ifpb,'(a)') '#           occ   : 1/0 put this function in the "occupied" group'
    write(ifpb,'(a)') '#           unocc : 1/0 put it in the "unoccupied" group'
    write(ifpb,'(a)') '#           The product basis is built from products (occupied group) x'
    write(ifpb,'(a)') '#           (unoccupied group), then pruned by pb_tolerance. phidot is'
    write(ifpb,'(a)') '#           usually left out (0 0) for speed; including it is more accurate.'
    write(ifpb,'(a)') '#'
    write(ifpb,'(a)') '# core    [iatom, l, n, occ, unocc, forX0, forSxc]   one row per core level'
    write(ifpb,'(a)') '#           n      : core index at this l (1 = deepest)'
    write(ifpb,'(a)') '#           occ/unocc : as above; forX0 / forSxc : 1/0 include this core in'
    write(ifpb,'(a)') '#           chi0 / in Sigma_xc (the CORE1/CORE2 distinction of PRB 76, 165106).'
    write(ifpb,'(a)') '#           Historical: modern runs leave every core row all-zero.'
    write(ifpb,'(a)')
