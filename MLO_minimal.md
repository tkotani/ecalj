...
- **FeMgO をサンプルに入れるのは保留** — 機械依存が残っているため。
  ...
  **同じ `rst.femgo`・同じ `efermi.lmf`・同じ入力なのに、ローカル(gfortran + Intel MPI)と
  ...
  | `atmpnu.*` / `__atm` | md5 まで一致。実行時に使われた pnu も全サイトで一致 |
  | 再現性 | **両機ともそれぞれの中では決定的**(同じ機で 2 回走らせるとバイト一致)。不定値ではない |
  ...
  **MLO の機構自体は両機とも正しい**(各機の中で完結させれば両方 1.6 / 0.9 meV)。
  ...
  ただし `smooth rhoves` の食い違いは $3.5\times10^{-4}$ Ry(97.590841 対 97.591193)で、
  $H$ のずれ 0.39 Ry より **3 桁小さい**。静電の小さな差では説明がつかない。

  > 当初「E_F がずれているのでは」と考えたが、`efermi.lmf` を揃えた
  > クリーンな実行でも $H$ は同じだけずれる。また「負の密度の扱いがコンパイラで違う」
  > とも考えたが、警告 20 件が完全一致していた。どちらも否定された。
  > 不定値でもない(両機とも決定的)。**原因はまだ特定できていない。**
