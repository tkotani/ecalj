...
#input string: cmd 
    ...
    def analyze_wvar0(self,name,tok):
...
#        self.analyze_wvar(tok,linenumber,fnlabel)
	...
	if iop==0:
		...
		if kind[0] in ['if','select','do','else','elseif']:
...
#input,string: line
	...
	while iloop>0:
		...
		if w_var[0] in tok and self.execfuncname[0]==w_var[6] :
			...
			if id>=0:
				accesspattern=self.analyze_wvar0(w_var[0],tok)
				for accesstype in accesspattern:
					self.indentmap.append([linenumber,[accesstype,w_var[0],w_var[6]],len(self.indentstack),fnlabel,tok])
					self.w_varlistundel_append_uniq(accesstype,w_var[0],w_var[6],linenumber,tok)
				if 'wref' in accesspattern:
					if self.printsource==1:
