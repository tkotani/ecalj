...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    !! ----- [product_basis] (slim: only the user-tuned scalars in GWinput.toml) -----
    ...
    write(ifi,'(a)')
    write(ifi,'(a)') '# Per-atom product-basis tables (nlx / valence / core) live in PB.'// &
         trim(sname)//'.toml'
    write(ifi,'(a)')
    ...
    !! ===== Write PB.<sname>.toml.tmp (per-atom product-basis tables) =====
    open(newunit=ifpb, file='PB.'//trim(sname)//'.toml.tmp')
    write(ifpb,'(a)') '# PB.'//trim(sname)//'.toml.tmp -- per-atom product basis tables.'
    write(ifpb,'(a)') '# Auto-generated by gwinit. Edit then rename to PB.'//trim(sname)//'.toml.'
    write(ifpb,'(a)') '# Loaded by m_GWinput in tandem with GWinput.toml.'
    write(ifpb,'(a)') '# (pb_tolerance and pb_lcutmx live in GWinput.toml; do not duplicate them here.)'
    write(ifpb,'(a)')
    ...
    close(ifpb)
    stop ' OK! GWinput.toml.tmp + PB.'//trim(sname)//'.toml.tmp generated. '// &
         'Rename both to drop the .tmp suffix after editing.'
