! PMT --1ststep--> MTP --2ndstep--> MLO. This is for 1ststep
program lmfham1 ! Get the Hamiltoniand on the MT-Projected orbitals <MTP|H|MTP> and overlap <MTP|MTP>
  use m_HamPMT,only: ReadHamPMTInfo, HamPMTtoHamRsMTP, plat,npair,nlat,nqwgt,ldim,nkp,qplist,ib_table,alat
  !                                  HamPMTtoHamRsMTP do not change variables here. Only generate HamRsMTO file.
  use m_HamRsMTP,  only: ReadHamRsMTP, hammr,ovlmr,ndimMTO, npairmx,nspx,ib_tableM
  ...
  call HamPMTtoHamRsMTP(facw,ecutw,eww) ! HamRsMTO (real-space Hamiltonian hammr,ovlmr,ndimMTO) is generated,and written to a file HamRsMTO
  call ReadHamRsMTP()                   ! Read real-space Hamiltonian hammr,ovlmr from HamRsMTO.
  if(symlcase) then
