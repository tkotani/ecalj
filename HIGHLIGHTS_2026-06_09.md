...
- **2026-09-19 16:30 追記 — 金属の QSGW が反復で荒れる正体が分かった（kBT §3.5）**: 小 q の W_c(ω) にある
  鋭いプラズモン極（LiTi₂O₄ では 1.8 eV、幅 0.1 eV）を、Σ_c の実軸極項が
  ω_ε = ε_qn − ε_{q−k,n′} の 1 点で拾い、反復のたびに極の反対側へ渡っていた。
  `[gw] wcsmear = true` で中間準位の smearing 核（esmr / kBT）を極の位置にも適用すると、
