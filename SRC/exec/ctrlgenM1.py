...
# Fortran reads ctrlg.<ext>.toml only; use ctrlgenToml.py
...
Ce  =" atomz=58@ pz='PZ=0,5,0,5'@ eh=-1*4@ eh2=-2*4@ R=?@ ext='IDU=0 0 0 12  UH=0 0 0 0.63   JH=0 0 0 0.0727  MMOM=0 0 0 1'@" 
...
Eu  =" atomz=63@ pz='PZ=0,5,0,5'@ eh=-1*4@ eh2=-2*4@ R=?@ ext='IDU=0 0 0 12  UH=0 0 0 0.69   JH=0 0 0 0.0815  MMOM=0 0 0 6'@" 
...
head= r"""# NOTE: 2025-8-21. Not for 5f. See https://ecalj.github.io/ecaljdoc
...
#           rrr = string.atof(rdic[speckey]) * touchingratio
    ...
    try:
        ...
        lmxa='      KMXA={kmxa} LMXA=4 NMCORE=1\n' 
        if( float(z)<1.5): lmxa='      KMXA={kmxa} LMXA=3 NMCORE=1\n' 
        if( float(z)>56.1 and float(z)<71.9): lmxa='      KMXA={kmxa} LMXA=6 NMCORE=1\n'
        print(' lmxa=',lmxa)
...
tail="""
\n"""
tail = tail+ "### You may touch only what you know. gwmax=12 is not need to touch \n"
