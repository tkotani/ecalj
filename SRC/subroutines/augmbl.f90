...
c     if(init) then
c     write(6,*)'fff lso=',lso
c     write(6,"('fff facso 1=',6(2f9.4,2x))")facso(:,1)
c     write(6,"('fff facso 2=',6(2f9.4,2x))")facso(:,2)
c     write(6,"('fff facso 3=',6(2f9.4,2x))")facso(:,3)
c     init=.false.
c     endif
      endif
...
c        print *,' np nh nsp=',np,nh,nsp,np*np*nsp
c        allocate(SSbPP(np*np*nsp),SSbHP(np*nh*nsp),SSbPH(np*nh*nsp),SSbHH(nh*nh*nsp)) 
...
c      use m_lmfinit,only: nlmto,iprmb
...
c      integer ltab(n0*nkap0),ktab(n0*nkap0),offl(n0*nkap0),norb
      double precision xx
      double complex cadd,cadd1
      complex(8),allocatable:: tso(:,:,:,:)
      call tcn ('augq2z')
C --- Loop over basis functions at site ia (augentation index) ---
c     call orbl(ia,0,nlmto,iprmb,norb,ltab,ktab,xx,offl,xx)
