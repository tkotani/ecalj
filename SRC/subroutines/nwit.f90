...
Co         :3 otherwise
Cu Updates
C   svflg :string of characters.  svflg(lsc+1) is written as the
C         :first character in the save file
...
C ... Local parameters
      ...
      if(eorb==0) then
      if (etot(1).ne.0d0) then
        i = i+1
        xv(i) = etot(1)*rydberg()
        sout=trim(sout)//',ehf(eV) '
      endif
      endif
      if (etot(2) .ne. 0d0) then
        i = i+1
        xv(i) = etot(2)*rydberg()
...
c      call iosave(flg,sout(2:),xv,-stdl,nvario)
      close(ifi)
      end
