...
contains
  ...
  subroutine gwinit_v2() bind(C)
    ...
    write(ifi,'(a)') '# ----- Smearing of the self-energy levels and electron temperature (manual/kBT) -----'
    write(ifi,'(a)') 't_sigmaw = 1000       # (K) width of the Fermi-Dirac kernel that smears the levels in Sigma (numerical;'
    write(ifi,'(a)') '                      # 1000 K ~ the old esmr=0.01 Ry). Metals: 300-1000 K with a finer k mesh.'
    write(ifi,'(a)') '# wcsmear  = true     # (default true) pole term of Sigma_c integrates W_c over the smeared level;'
    write(ifi,'(a)') '#                     # false = old 3-point interpolation at the mean energy'
