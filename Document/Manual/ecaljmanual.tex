...
First, see README.org shown at 
\url{https://github.com/tkotani/ecalj#ecalj-} 
(or ecalj/README.org in the package).
...
(also need to set \verb+DYN+ category given in Document/Manual/CategoryAndToken.org).
...
in python 3 and call fortran programs(lmfa,lmchk) internally).
...
\item Lattice info, Space group symmetry operations (in lmf format), and
      ...
      See SYMGRP in Document/Manual/CategoryAndToken.org.
...
\item
      C-HQ means that corehole is −１１. Thus we have only 14-11=3
...
It is possible to deform unit cell by adding some optional tokens under STRUC category.
Search STRUC in an old document (which may be still effective) BACKUP/MarksOriginalDoc/lmto.html.
However, it is a little complicated. 
...
example, and read DYN in Document/Manual/CategoryAndToken.org.
...
in Document/BACKUP/MarksOriginalDoc/lmto.html.
This is a little confusing, thus we will explain this in appendix. See Sec.\ref{pzsetting}.\\
