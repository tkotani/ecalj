...
module m_lmfham2
  ...
  private
contains
  subroutine lmfham2(commin) bind(C) ! Get the Hamiltonian on the MT Localized orbitals |MLO> from 
    ! that of the MT Localized zero |MLO0>. Conversion from MLO0(hmmr1,ommr1,nmto) to MLO(hmmr2,ommr2,nMLO).
    ! In advance to run lmfham2, run lmfham1 to get |MLO0> (MLO0 are given by a projection from MTOs to PMT space (removing APW).
    ! That is, |MLO0_i> = M |PMT_i>, where M is a mapping from PMT spece to MLO0 space.
    !
    ...
    ! Among the space spanned by |MLO0>, we extract minimum basis space.
    ! For example, we have 25+25 |MLO0> for Si; then we extract 9+9 space spanned by |MLO>.
    !
    ...
    use m_HamRsMLO,only: ReadHamRsMLO,hmmr1=>hammr,ommr1=>ovlmr,nmto=>ndimMTO,ib_tableM,l_tableM,k_tableM !Real-space Hamiltonian on the basis |MPO>.
    ...
    call ReadHamRsMLO()  ! Read Real-space |MTo>-basis Hamiltonian and overlap.
    call pshpr(30)
    call readqplistsy()
    hartree=2d0*rydberg()
    tpia = 2d0*pi/alat
...
2010 GetCNmatFile_job0: block  !job=0 mode to get CNmat file (connection matrix uumat and so on).
      ...
      complex(8):: emat(nmto,nmto),osq(1:nmto,1:nmto),o2al(1:nmto,1:nmto,nqbz),phase,ovlmm(nmto,nMLO),&
           ...
           ovlm(1:nmto,1:nmto),ovlmx(1:nmto,1:nmto), hamm(1:nmto,1:nmto),uumat(iki:ikf,iki:ikf,nbb,nqibz)
      allocate(evl(nmto,nqbz),ovl(nmto),eveci(1:nmto,1:nmto,nqbz)) !NOTE: 20230805. When I declear evl in this block, ifort18.05 gives wrong results.
