...
#    print '---- '+ffile+' start -----'
    ...
    for c in walk(parser.block):
        ins=c[0]
        depth=c[1]
        if(isinstance(ins, classes.Comment)): 
            #print dir(ins.content)
            print 'cccccccc@ ',ins.item.span,ins.item.comment #,ins.item.reader
            continue
...
##########
    for c in walk(parser.block):
        ...
        print loc,item.strline,item.strlinemap
        deptho=depth
        inso=ins
        continue
##########################################3
