program hx0zmel ! Calculate matrix elements zmel needed for calculating x0.
  ...
  use m_rdpp,only: Rdpp, nxx,lx,nx,mdimx,nbloch ,nblochpmx,mrecl,nprecx !! Base data to generate matrix elements zmel*. Used in "call get_zmelt".
  ...
  use m_x0kf,only:    X0kf_v4hz,  X0kf_v4hz_init, X0kf_v4hz_init_write,X0kf_v4hz_init_read,X0kf_zmel, kc,ncount !& !X0kf_v4hz_symmetrize,
  ...
  realomega = .true.
  imagomega = .true.
  call Getfreq2(.false.,realomega,imagomega,ua,iprintx)! frhis,freq_r,freq_i, nwhis,nw,npm
  ...
  iqloop: do 2501 iq = iqxini,iqxend !!! Get zmel
     if( .NOT. MPI__Qtask(iq) ) cycle
     kold=-999
     isold=-999
     qp = qibze(:,iq)
     write(stdo,"('do 1501: iq q=',i5,3f9.4)")iq,qp
...
2001    enddo icountloop
2003 enddo isloop
2501 enddo iqloop
  write(stdo,*) '--- end of hx0zmel --- irank=',MPI__rank
  call cputid(0)
  call rx0( ' OK! hx0zmel')
end program hx0zmel
